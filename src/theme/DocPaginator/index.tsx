  ...
  function popover() {
    return (
      <div style={{width: '390px', textAlign: 'center'}}>
        <div style={{marginBottom: '8px', textAlign: 'left'}}>{$t('Leave an optional comment…')}</div>
        <TextArea value={commentValue} onChange={(e)=> setCommentValue(e?.target?.value)} rows={5}></TextArea>
        <Button 
        loading={loading}
        onClick={()=> submitInfo('No')} style={{marginTop: '16px', fontWeight: 700}} type='primary'>{$t('Vote')}</Button>
      </div>
    )
  }
