...
Databend HTTP Handler 是一个 REST API，用于发送查询语句以便在服务器上执行，并将结果返回给客户端。
...
此 Handler 以 `pages` 形式返回结果，并采用长轮询方式。

1. 首先，使用 `POST` 方法向 `/v1/query` 发送一个 JSON 格式的 `QueryRequest`，其中包含要执行的 SQL 语句，返回一个 JSON 格式的 `QueryResponse`。
2. 使用 `QueryResponse` 的字段进行进一步处理：
   1. 使用 `GET` 方法访问 `next_uri`，返回查询结果的下一个 `page`。它也返回 `QueryResponse`，以相同的方式处理，直到 `next_uri` 为 null。
   2. （可选）使用 `GET` 方法访问 `kill_uri` 以终止查询。返回空 body。
   3. （可选）使用 `GET` 方法访问 `stats_uri` 以仅获取一次统计信息（不使用长轮询），返回 `QueryResponse`，其中 `data` 字段为空。

请注意，在查询完成之前，您应持续使用最新的 `next_uri` 来获取下一页结果，否则可能会错过一些结果或泄漏会话资源，直到会话超时。当您收到查询的所有结果时，`next_uri` 将为 null。
...
| 字段            | 类型          | 描述                                                                                              |
| --------------- | ------------- | ------------------------------------------------------------------------------------------------- |
| running_time_ms | float         | 自查询开始在内部执行以来经过的毫秒数，在查询完成时停止计时（state != Running）                   |
...
| 代码 | 错误                                                                         |
| ---- | ---------------------------------------------------------------------------- |
| 200  | 如果 SQL 无效或失败，则详细信息位于 JSON 的 `error` 字段中                   |
| 404  | 未找到 "query_id" 或 "page"                                                 |
| 400  | 无效的请求格式                                                               |

当状态代码不是 200 时，请检查响应正文中的错误原因（作为字符串）。
...
如果 SQL 是 `set` 或 `use`，则 `session` 将更改，可以在响应中传回给客户端，
客户端需要记录它并将其放入后续请求中。

### QueryAffect (实验性)

对于每个 SQL，客户端都会获得一个可选的表格形式的 `result`。
客户端还可以获得有关行/字节读取/写入的 `progress` 信息。
由于它们的限制，客户端可能无法获得有关查询的所有有趣信息。
因此，我们添加了 `QueryAffect` 以携带有关查询的一些额外信息。
...
HTTP handler 客户端实现的最简单示例位于 [sqllogictest](https://github.com/databendlabs/databend/blob/main/tests/sqllogictests/src/client/http_client.rs) 中，用于 databend。
