...
Databend Go 驱动程序与 ["database/sql"](https://pkg.go.dev/database/sql) 接口规范兼容。下面是一些常见的基本行为，以及涉及的关键函数和背后的原理。
...
| 批量插入              | `DB.Begin`, `Tx.Prepare`, `Stmt.Exec`, `Tx.Commit` | 通过事务处理批量插入/替换数据（`INSERT INTO` 和 `REPLACE INTO`）。<br /><br />使用 `Stmt.Exec` 尽可能多地向预处理语句对象添加数据；数据将被追加到一个文件中。<br /><br />执行 `Tx.Commit()` 最终会将数据上传到内置 Stage 并执行插入/替换操作，使用 [Stage 附件](/developer/apis/http#stage-attachment)。 |
...
| 上传到内部 Stage      | `APIClient.UploadToStage`                          | 上传数据到 Stage。默认情况下，使用 `PRESIGN UPLOAD` 获取一个 URL，或者如果 PRESIGN 被禁用，使用 `v1/upload_to_stage` API。 |
...
要连接您的程序到 Databend 并执行 SQL 操作，您必须在代码中提供具有适当权限的 SQL 用户账户。如有需要，在 Databend 中创建一个，并确保 SQL 用户仅具有出于安全考虑所需的权限。
...
:::note
- 下面的代码连接到一个本地 Databend，使用名为 'user1' 和密码 'abc123' 的 SQL 用户作为示例。在保持相同格式的同时，随意使用您自己的值。
...
require (
	github.com/BurntSushi/toml v1.2.1 // 间接
	github.com/avast/retry-go v3.0.0+incompatible // 间接
	github.com/google/uuid v1.3.0 // 间接
	github.com/pkg/errors v0.9.1 // 间接
	github.com/sirupsen/logrus v1.9.0 // 间接
	golang.org/x/sys v0.5.0 // 间接
)
```
...
## 教程-2：使用 Golang 集成 Databend Cloud

在开始之前，请确保您已成功创建仓库并获取了连接信息。有关如何做到这一点，请参阅[连接到仓库](/guides/cloud/using-databend-cloud/warehouses#connecting)。
...
:::tip
将代码中的 {USER}、{PASSWORD}、{HOST}、{WAREHOUSE_NAME} 和 {DATABASE} 替换为您的连接信息。有关如何获取连接信息，
请参阅[连接到仓库](/guides/cloud/using-databend-cloud/warehouses#connecting)。
:::
