...
在本教程中，我们将模拟在本地生成日志，使用 [Vector](https://vector.dev/) 收集它们，将它们存储在 S3 中，并使用计划任务自动将它们摄取到 Databend Cloud 中。
...
- **Docker**: 确保您的本地计算机上已安装 [Docker](https://www.docker.com/)，因为它将用于设置 Vector。
...
1. 在您的本地计算机上创建一个名为 `vector.yaml` 的 Vector 配置文件。在本教程中，我们在 `/Users/eric/Documents/vector.yaml` 中创建它，内容如下：
...
与原始日志相比，同步的日志采用 NDJSON 格式，每个记录都包含在外部 `log` 字段中：
...
稍等片刻，让日志同步到 S3（`logs` 文件夹中应出现一个新文件）。然后，计划任务会将新日志加载到表中。如果您再次查询该表，您将找到这些日志：
