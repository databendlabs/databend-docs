...
在本教程中，我们将加载、分析并创建一个名为“来自纽约时报的 Covid-19 数据”的数据集的仪表盘。该数据集包含每日更新的关于美国 Covid-19 病例、死亡和其他相关统计数据的信息。它提供了对疫情在美国全国、州和县各级影响的全面观察，提供了对 2022 年全年各个地区 Covid-19 情况的详细见解。
...
此 SQL 语句返回 `0`，表示表中没有重复的行，现在数据已准备好进行分析。
