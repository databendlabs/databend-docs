...
> **Note**:
> 如果您的 `user` 或 `password` 包含特殊字符，您需要在相应的字段中单独提供它们（例如，DBeaver 中的 `Username` 和 `Password` 字段）。在这种情况下，Databend 将为您处理必要的编码。但是，如果您一起提供凭据（例如，作为 `user:password`），则必须确保在使用前对整个字符串进行正确编码。
...
3. 在 **Driver properties** 选项卡中，根据上一步中获得的连接信息输入 **计算集群** 名称。
