...
- 如果 Meta Client 开始使用 Meta Server 提供的新的 API，请将 `MIN_METASRV_SEMVER` 升级到引入此新 API 的构建版本。
- 如果 Meta Server 删除了一个 API，请将 `MIN_METACLI_SEMVER` 升级到自不再使用此 API 以来的客户端构建版本。
...
[兼容性](/guides/deploy/upgrade/compatibility)
