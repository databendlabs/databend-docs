...
2. 获取用于连接到您创建的存储桶或容器的 endpoint URL。
3. 获取您帐户的 Access Key ID 和 Secret Access Key。

有关如何管理云对象存储的存储桶和 Access Key 的信息，请参阅解决方案提供商的用户手册。以下是一些您可能需要的有用链接：
...
上面的命令将生成一个名为 `base64-encoded-key.txt` 的文件，其中包含您随后用于在 `databend-query.toml` 配置文件中配置连接的凭据。
...
有关如何管理 MinIO 的存储桶和 Access Key 的信息，请参阅解决方案提供商的用户手册。以下是一些您可能需要的有用链接：
...
3. 获取用于连接到您创建的存储桶或容器的 endpoint URL。
4. 获取您帐户的 Access Key ID 和 Secret Access Key。

有关如何管理 CubeFS 的存储桶和 Access Key 的信息，请参阅解决方案提供商的用户手册。以下是一些您可能需要的有用链接：
...
1. 找到文件夹 `/usr/local/databend/configs` 中的文件 `databend-query.toml`。
2. 在文件 `databend-query.toml` 中，设置 [storage] 块中的参数 *type*，并配置用于连接到您的对象存储的访问凭据和 endpoint URL。

要配置您的存储设置，请通过在每行开头添加 `#` 来注释掉 [storage.fs] 部分。然后，通过删除 `#` 符号来取消注释与您的对象存储提供商相关的部分，并填写您的值。
