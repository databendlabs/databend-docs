...
| 函数                                      | 描述                                                                                      | 示例                                         | 结果                      |
|-------------------------------------------|-------------------------------------------------------------------------------------------|----------------------------------------------|---------------------------|
| **GET(array, index)**                     | 通过索引返回数组中的元素（基于1）                                                         | **GET([1, 2], 2)**                           | 2                         |
| **LENGTH(array)**                         | 返回数组的长度                                                                            | **LENGTH([1, 2])**                           | 2                         |
| **RANGE(start, end)**                     | 返回由 [start, end) 收集的数组                                                            | **RANGE(1, 3)**                              | [1, 2]                    |
| **ARRAY_CONCAT(array1, array2)**          | 合并两个数组                                                                              | **ARRAY_CONCAT([1, 2], [3, 4])**             | [1,2,3,4]                 |
...
| **ARRAY_SLICE(array, start[, end])**      | 通过索引提取数组的片段（基于1）                                                           | **ARRAY_SLICE([1, 21, 32, 4], 2, 3)**        | [21,32]                   |
...
:::note
**ARRAY_SORT(array)** 可以接受两个可选参数，`order` 和 `nullposition`，可以通过 **ARRAY_SORT(array, order, nullposition)** 语法指定。
   - `order` 指定排序顺序，可以是升序（ASC）或降序（DESC）。默认为ASC。
   - `nullposition` 确定排序结果中NULL值的位置，在排序输出的开始（NULLS FIRST）或结束（NULLS LAST）。默认为NULLS FIRST。
:::
