...
- [在 Docker 上部署 Databend](#在-docker-上部署-databend)：您可以在 Docker 上部署 Databend 和 [MinIO](https://min.io/)，以实现容器化设置。

- [部署本地 Databend](#部署本地-databend)：如果没有对象存储可用，您可以选择本地部署，并使用文件系统作为存储。
...
```shell
bendsql -h 127.0.0.1 -P 3307 -u databend -p databend
```

现在，您已成功在 Docker 上部署并连接到 Databend。

## 部署本地 Databend

如果您没有对象存储可用，您可以选择在本地部署 Databend。以下是在本地部署 Databend 的步骤：

### 步骤 1. 下载 Databend 二进制文件

您可以从 [Databend GitHub 仓库](https://github.com/datafuselabs/databend/releases) 下载最新的 Databend 二进制文件。
...
```shell
./databend-query -c configs/config_query.toml
```

3. 另外打开一个终端窗口，运行以下命令启动 Databend Meta 服务：

```shell
./databend-meta -c configs/config_meta.toml
```

### 步骤 3. 连接到 Databend

与在 Docker 上部署时一样，您可以使用 BendSQL CLI 工具与本地部署的 Databend 建立连接。运行以下命令：

```shell
bendsql -h 127.0.0.1 -P 3307 -u root
```

现在，您已成功在本地部署并连接到 Databend。
```
...
尝试连接到 localhost:8000 作为用户 root。
