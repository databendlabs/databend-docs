...
这个工作流的吞吐量受限于databend的HTTP API：`upload_to_stage`。我们可以通过允许用户直接上传到我们的后端存储来提高吞吐量。例如，我们可以使用[AWS认证请求：使用查询参数](https://docs.aws.amazon.com/AmazonS3/latest/API/sigv4-query-string-auth.html)来生成预签名URL。这样，用户可以直接上传内容到AWS s3，而不需要经过databend。
