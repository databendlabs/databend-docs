...
Databend 中的存储过程（Stored Procedure）允许您将 SQL 逻辑打包，在服务器上运行，并可以访问控制流（Control Flow）、变量（Variable）、游标（Cursor）和动态语句（Dynamic Statement）。本页介绍如何创建过程以及编写为其提供支持的内联脚本。
...
存储过程（Stored Procedure）可以在可执行部分之前使用一个可选的 `DECLARE` 块来初始化变量。
...
Lambda 表达式定义了可以传递给数组函数或在查询中调用的内联逻辑。它们遵循 `<parameter> -> <expression>` 的形式（当提供多个参数时，用括号将参数括起来）。表达式可以包括类型转换、条件逻辑，甚至对过程变量的引用。

- 当 lambda 在 SQL 语句中运行时，使用 `:variable_name` 来引用 lambda 内部的过程变量。
- 像 `ARRAY_TRANSFORM` 和 `ARRAY_FILTER` 这样的函数会对输入数组中的每个元素评估 lambda。
...
您也可以在 lambda 主体内部放置复杂的表达式，例如 `CASE` 逻辑。
...
基于范围的循环从下限迭代到上限（含）。使用可选的 `REVERSE` 关键字来反向遍历范围。
...
`FOR` 也可以遍历先前声明的结果集变量或游标（Cursor）（参见[处理查询结果](#working-with-query-results)）。
...
结果集变量和游标（Cursor）可以使用 `FOR ... IN` 循环进行遍历。
...
使用 `RETURN TABLE(<query>)` 来输出一个表格结果。
...
`EXECUTE IMMEDIATE` 的调用者会收到动态块返回的结果。在块内部使用 `RETURN TABLE` 来生成结果集。
