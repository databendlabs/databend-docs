...
SETTINGS Clause 用于配置影响其前面的 SQL 语句执行行为的特定设置。要查看 Databend 中可用的设置及其值，请使用 [SHOW SETTINGS](../50-administration-cmds/03-show-settings.md)。
...
-- 将时区设置为 America/Toronto 会将 now() 输出调整为多伦多的本地时间，反映东部时区（UTC-5 或夏令时期间的 UTC-4）。
