...
参见：[Limit Clause](01-query-select.md#limit-clause)
...
- `TOP` 和 `LIMIT` 是用于限制查询返回行数的等效关键字，但它们不能在同一个查询中一起使用。
- 如果 `TOP` 在没有 `ORDER BY` 子句的情况下使用，则查询缺少选择顶部行的有意义的序列，可能导致不一致或意外的结果。
