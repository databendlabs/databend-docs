...
通过 CHANGES 子句，可以查询定义时间间隔内表的变更追踪元数据。请注意，时间间隔必须在数据保留期内（默认为 24 小时）。要定义时间间隔，请使用 `AT` 关键字指定时间点作为间隔的开始，当前时间将用作间隔的默认结束时间。如果希望指定过去的时间作为间隔的结束时间，请结合使用 `END` 关键字和 `AT` 关键字来设置间隔。
...
CHANGES 子句要求在表上将 Fuse engine option `change_tracking` 设置为 `true`。有关 `change_tracking` 选项的更多信息，请参见 [Fuse Engine Options](/sql/sql-reference/table-engines/fuse#options)。
