...
- 使用 `error_on_nondeterministic_update = true`，此查询失败，因为目标中的 id = 2 匹配源中的多个行，从而使更新不明确。
...
- 使用 `error_on_nondeterministic_update = false`，更新成功，但 id = 2 的 target.price 可能会更新为 399.99 或 349.99，具体取决于执行顺序。
...
在此步骤中，我们创建第二个名为 **book_updates** 的表，该表保存更新后的书名，我们将使用它来更新 **bookstore** 表。
