...
- User / Internal / External stages: 请参阅 [What is Stage?](/guides/load-data/stage/what-is-stage) 了解 Databend 中的 stages。
- 在存储服务中创建的存储桶或容器。
...
有关访问类似 Amazon S3 的存储服务可用的连接参数，请参阅 [Connection Parameters](/00-sql-reference/51-connect-parameters.md)。
</TabItem>
...
有关访问 Azure Blob Storage 可用的连接参数，请参阅 [Connection Parameters](/00-sql-reference/51-connect-parameters.md)。
</TabItem>
...
有关访问 Google Cloud Storage 可用的连接参数，请参阅 [Connection Parameters](/00-sql-reference/51-connect-parameters.md)。
</TabItem>
...
有关访问 Alibaba Cloud OSS 可用的连接参数，请参阅 [Connection Parameters](/00-sql-reference/51-connect-parameters.md)。
</TabItem>
...
有关访问 Tencent Cloud Object Storage 可用的连接参数，请参阅 [Connection Parameters](/00-sql-reference/51-connect-parameters.md)。
</TabItem>
...
有关访问 HDFS 可用的连接参数，请参阅 [Connection Parameters](/00-sql-reference/51-connect-parameters.md)。
</TabItem>
...
有关访问 WebHDFS 可用的连接参数，请参阅 [Connection Parameters](/00-sql-reference/51-connect-parameters.md)。
</TabItem>
</Tabs>

### FILE_FORMAT

有关详细信息，请参阅 [Input & Output File Formats](../../00-sql-reference/50-file-format-options.md)。
...
| USE_RAW_PATH     | false                  | 如果为 `true`，则将使用用户提供的确切路径（包括完整文件名）导出数据。如果设置为 `false`，则用户必须提供目录路径。                                                                                              |

### DETAILED_OUTPUT

确定是否应返回数据卸载的详细结果，默认值设置为 `false`。有关更多信息，请参见 [Output](#output)。
...
当 `DETAILED_OUTPUT` 设置为 `true` 时，COPY INTO 提供包含以下列的结果。这有助于定位卸载的文件，尤其是在使用 `MAX_FILE_SIZE` 将卸载的数据分隔为多个文件时。
...
### 示例 1：卸载到 Internal Stage

此示例将数据卸载到 internal stage：
