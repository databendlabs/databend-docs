...
在此示例中，INSERT OVERWRITE 语句用于截断 employee 表并插入新数据，用 ID 为 100 的 employee 的值替换所有现有记录。
...
此示例演示如何创建一个名为 "sales_summary" 的摘要表，用于存储每个产品的聚合销售数据，例如总销售数量和收入，方法是聚合 sales 表中的信息：
...
通过 INSERT INTO 语句，Databend 允许您将数据从 Stage 文件插入到表中。这是通过 Databend 的[查询 Stage 文件](/guides/load-data/transform/querying-stage)的能力实现的，随后将查询结果合并到表中。
...
3. 使用 `INSERT INTO` 从 Stage 的 Parquet 文件中插入数据
