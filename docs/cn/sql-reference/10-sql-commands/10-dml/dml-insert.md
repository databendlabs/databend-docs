...
此示例演示如何创建一个名为“sales_summary”的汇总表，用于存储每个产品的汇总销售数据，例如总销售数量和收入，方法是汇总 sales 表中的信息：
...
此示例演示如何创建一个名为“staff_records”的表，并为 department 和 status 等列设置默认值。然后插入数据，展示默认值的使用。

```sql
-- 创建一个带有列“employee_id”、“department”、“salary”和“status”的表“staff_records”，并带有默认值
...
### 示例-4：插入 Staged Files

通过 Databend，您可以使用 INSERT INTO 语句将数据从 Staged Files 插入到表中。这是通过 Databend 的[查询 Staged Files](/guides/load-data/transform/querying-stage)的能力实现的，随后将查询结果合并到表中。
...
3. 使用 `INSERT INTO` 从 Staged Parquet 文件中插入数据
