...
REPLACE INTO 语句可以将多个新行插入到表中，或者在这些行已存在时更新现有行，数据来源包括：

- 直接值

- 查询结果

- Staged files: Databend 允许您使用 REPLACE INTO 语句从 staged files 中替换表中的数据。这是通过 Databend 的 [Query Staged Files](/guides/load-data/transform/querying-stage) 功能实现的，随后将查询结果合并到表中。
...
此示例演示如何使用 staged file 中的数据替换表中的现有数据。
