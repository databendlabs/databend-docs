...
Databend 中的 Connection 是指一种指定的配置，它封装了与外部存储服务交互所需的详细信息。它充当一组集中且可重用的参数，例如访问凭据、端点 URL 和存储类型，从而促进 Databend 与各种存储服务的集成。

Connection 可用于创建外部 Stage、外部表和附加表，从而提供一种简化的模块化方法来管理和访问存储在外部存储服务中的数据（通过 Databend）。 有关示例，请参见 [使用示例](#usage-examples)。

### 管理 Connection

要在 Databend 中管理 Connection，请使用以下命令：
...
此示例演示了如何使用先前定义的名为“toronto”的 Connection 创建名为“BOOKS”的外部表：
