...
创建一个SQL用户，提供用户的名称、认证类型和密码等详细信息。您还可以选择指定一个[网络策略](../12-network-policy/index.md)并为用户设置一个[默认角色](/guides/security/access-control#setting-default-role)。

另请参阅：

 - [创建网络策略](../12-network-policy/ddl-create-policy.md)
 - [授予用户权限](./10-grant-privileges.md)
 - [授予用户角色](./20-grant-role.md)

## 语法

```sql
CREATE USER <name> IDENTIFIED [WITH <auth_type> ] BY '<password>' 
[WITH SET NETWORK POLICY='<network_policy>'] -- 设置网络策略
