...
Cluster key 旨在通过将数据物理地聚集在一起来提高查询性能。例如，当您将一列设置为表的 cluster key 时，表数据将按您设置的列进行物理排序。如果您的多数查询都按该列进行过滤，这将最大限度地提高查询性能。

> **注意：** 对于 String 列，集群统计信息仅使用前 8 个字节。您可以使用子字符串来提供足够的基数。
