...
| AT                  | 当使用 `AT` 后跟 `TIMESTAMP =>` 或 `SNAPSHOT =>` 时，您可以通过时间戳或快照 ID 创建一个包含特定历史点之后的数据更改的 Stream；当 `AT` 后跟 `STREAM =>` 时，它允许创建与现有 Stream 相同的新 Stream，保留相同的捕获数据更改。 |
| APPEND_ONLY         | 设置为 `true` 时，Stream 在 `Append-Only` 模式下运行；设置为 `false` 时，Stream 在 `Standard` 模式下运行。默认为 `true`。有关 Stream 操作模式的更多详细信息，请参见 [Stream 工作原理](/guides/load-data/continuous-data-pipelines/stream#how-stream-works)。                                        |
...
以下示例创建一个名为“order_changes_copy”的新 Stream，其中包含 `AT` 参数，其中包含与“order_changes”相同的数据更改：
