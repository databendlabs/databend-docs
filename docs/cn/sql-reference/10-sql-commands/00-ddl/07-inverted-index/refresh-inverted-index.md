...
- 当在创建倒排索引之前将数据插入到表中时，创建后需要手动刷新倒排索引，以便有效地索引插入的数据。
- 当倒排索引遇到问题或损坏时，需要刷新它。如果由于某些块的倒排索引文件损坏而导致倒排索引中断，则诸如 `where match(body, 'wiki')` 之类的查询将返回错误。在这种情况下，您需要刷新倒排索引以解决此问题。
