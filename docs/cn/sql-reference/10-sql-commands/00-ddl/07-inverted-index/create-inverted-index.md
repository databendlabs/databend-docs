...
| `english_stemmer` | 将同一单词的不同形式映射到一个通用单词。例如，"walking" 和 "walked" 将映射到 "walk"。                                                                   |
...
| `basic`      | 否      | 仅存储 DocId，占用空间最小，但不支持 "brown fox" 等短语搜索。                                                                                              |
| `freq`       | 否      | 存储 DocId 和词频，占用空间中等，不支持短语术语，但可以提供更好的分数。                                                                                              |
