...
| 参数                 | 描述                                                                                                                              | 是否必需 |
|----------------------|-----------------------------------------------------------------------------------------------------------------------------------|----------|
| `SIZE_LIMIT = <num>` | 指定给定 COPY 语句要加载的最大数据行数的数字（> 0）。 默认为 `0`                                                                       | 可选     |
| `PURGE = <bool>`     | 如果文件成功加载到表中，则 True 指定该命令将清除 Stage 中的文件。 默认为 `false`                                                      | 可选     |
...
5. 为用例选择 **第三方服务**，然后勾选下面的复选框以确认访问密钥的创建。
6. 复制生成的访问密钥和秘密访问密钥并将其保存到安全的地方。
...
2. 单击 **创建存储桶** 以创建一个存储桶，并将存储桶名称设置为 *databend*。 成功创建存储桶后，当您查看存储桶详细信息页面时，可以在存储桶名称正下方找到存储桶端点。
...
4. 单击 **创建 API 令牌** 以获取访问密钥 ID 和秘密访问密钥。 复制并将其保存到安全的地方。

#### 步骤 3：创建外部 Stage

使用创建的访问密钥 ID 和秘密访问密钥创建一个名为 *r2_stage* 的外部 Stage。
