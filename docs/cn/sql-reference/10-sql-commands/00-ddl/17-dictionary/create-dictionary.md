...
|-----------|-----------|----------------------------------------------------------------------------------|
| host      | Yes       | MySQL 服务器的 IP 地址或主机名。                                  |
| port      | Yes       | MySQL 服务器正在侦听的端口。                                 |
| username  | Yes       | 用于连接到 MySQL 服务器的用户名。                                |
| password  | Yes       | 与用户名关联的用于访问 MySQL 服务器的密码。            |
| db        | Yes       | 要从中提取数据的 MySQL 服务器上的数据库的名称。 |
| table     | Yes       | 数据库中数据所在的表的名称。                    |
...
| Parameter | Required? | Description                                                                                                                                 |
|-----------|-----------|-----------------------------------------------------------------------------------------------------------------------------|
| host      | Yes       | Redis 服务器的主机名或 IP 地址。                                                                                             |
| port      | Yes       | Redis 服务器的端口号。                                                                                                        |
| username  | No        | 如果 Redis 服务器需要用户身份验证，则为用户名。                                                                                  |
| password  | No        | 用于用户身份验证的密码。                                                                                                       |
| db_index  | No        | 指定 Redis 数据库索引，默认为 0。索引范围从 0 到 15，因为 Redis 支持从 0 到 15 索引的 16 个数据库。 |
