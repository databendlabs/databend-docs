...
- `LIMIT <file_count>`：此参数可以与 DRY RUN 参数一起使用，也可以不与 DRY RUN 参数一起使用。与 DRY RUN 一起使用时，它会限制在 `DRY RUN` 结果中显示的数据文件数量。在没有 `DRY RUN` 的情况下使用时，它限制了要清理的数据文件的数量。
...
- `DRY RUN SUMMARY`：返回要删除的文件总数以及每个已删除表的总大小。
...
| Databend Cloud (基础版)                | 1 day (24 hours)  | 1 day (24 hours) |
