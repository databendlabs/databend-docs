...
- **通用表（默认）**：这些表本身支持时间回溯，允许您跟踪和检索历史数据。此功能对于数据分析和审计很有价值。
...
Databend 默认将表数据存储在 [databend-query.toml](https://github.com/databendlabs/databend/blob/main/scripts/distribution/configs/databend-query.toml) 配置文件中配置的位置。此外，它还提供了将表数据存储在不同 bucket 中的灵活性，从而偏离了默认设置。有关更多信息，请参见 [CREATE TABLE ... EXTERNAL_LOCATION](10-ddl-create-table.md#create-table--external_location)。
