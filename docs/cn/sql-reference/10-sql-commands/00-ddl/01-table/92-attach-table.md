...
将现有表附加到另一个表。该命令将一个数据库中的表的数据和架构移动到另一个数据库，但实际上不复制数据。相反，它创建一个指向原始表数据的链接，用于访问数据。

Attach Table 使您能够无缝地将云服务平台中的表连接到部署在私有部署环境中的现有表，而无需物理移动数据。当您想要将数据从 Databend 的私有部署迁移到 [Databend Cloud](https://www.databend.com) 同时最小化数据传输开销时，这特别有用。

Attach Table 在源表和附加表之间提供无缝连接，数据同步机制的操作方式根据模式的不同而有所不同：
...
- `<source_table_data_URI>` 代表源表数据的路径。对于类 S3 的对象存储，格式为 `s3://<bucket-name>/<database_ID>/<table_ID>`，例如，*s3://databend-toronto/1/23351/*，它代表桶内表文件夹的确切路径。

  ![Alt text](@site/docs/public/img/sql/attach.png)

  要获取表的数据库 ID 和表 ID，请使用 [FUSE_SNAPSHOT](../../../20-sql-functions/16-system-functions/fuse_snapshot.md) 函数。在下面的示例中，*snapshot_location* 的值中的部分 **1/23351/** 表明数据库 ID 是 **1**，表 ID 是 **23351**。
...
- `CONNECTION` 指定建立链接到源表数据存储的对象存储所需的连接参数。连接参数根据不同存储服务的具体要求和认证机制而有所不同。更多信息，请参见 [连接参数](../../../00-sql-reference/51-connect-parameters.md)。
...
使用 [FUSE_SNAPSHOT](../../../20-sql-functions/16-system-functions/fuse_snapshot.md) 函数获取数据库 ID 和表 ID。下面的结果表明数据库 ID 是 **1**，表 ID 是 **216**：
...
为了验证链接成功，请在 Databend Cloud 中运行以下查询：
...
一切就绪！如果您在 Databend 中更新源表，您可以观察到目标表格在 Databend Cloud 上的相同更改。例如，如果您将源表 "employees" 中的名字 "Mike Johnson" 更新为 "Eric Johnson"：
...
您可以看到更新同步到 Databend Cloud 中的附加表格：
...
您也可以更新 Databend Cloud 中的目标表格，但更改不会同步到 Databend 中的源表格。例如，如果您在目标表格 "employees_backup" 中将 "John Doe" 的薪水更新为 5500：
...
### 示例 2：以 READ_ONLY 模式链接表格

此示例说明如何在 Databend Cloud 中以 READ_ONLY 模式链接一个新表格，该表格与存储在名为 "databend-toronto" 的 Amazon S3 桶中的现有表格相连。

#### 步骤 1. 在 Databend 中创建表格

创建一个名为 "population" 的表格并插入一些样本数据：
...
#### 步骤 2. 获取数据库 ID 和表格 ID

使用 [FUSE_SNAPSHOT](../../../20-sql-functions/16-system-functions/fuse_snapshot.md) 函数获取数据库 ID 和表格 ID。下面的结果表明数据库 ID 为 **1**，表格 ID 为 **556**：
...
一切就绪！如果你在 Databend 中更新源表，你可以在 Databend Cloud 的目标表中看到相同的更改反映出来。例如，如果你将 Toronto 的人口数更改为 2,371,571 在源表中：
