...
以下示例显示当前数据库中表的状态，提供诸如名称、引擎、行和其他相关信息的详细信息：
...
:::note
使用 SHOW TABLE STATUS 查询时，请注意某些列名（例如 "rows"）可能会被解释为 SQL 关键字，从而可能导致错误。为避免此问题，请始终用反引号将列名括起来，如本示例所示。这确保了列名在 SQL 查询中被视为标识符而不是关键字。
:::
