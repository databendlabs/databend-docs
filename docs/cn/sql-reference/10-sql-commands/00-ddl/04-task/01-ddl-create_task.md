...
| [ERROR_INTEGRATION](../16-notification/index.md) | 可选。用于 task 错误通知的通知集成的名称，应用了特定的 [task 错误 payload ](./10-task-error-integration-payload.md)                                                               |
...
- 必须为独立的 task 或 task DAG 中的根 task 定义时间表；否则，task 仅在手动使用 EXECUTE TASK 执行时运行。
...
- 多个从单个表 stream 消耗更改数据的 task 检索不同的增量。当 task 使用 DML 语句消耗 stream 中的更改数据时，stream 会提前偏移量。更改数据不再可供下一个 task 消耗。目前，我们建议只有一个 task 消耗 stream 中的更改数据。可以为同一张表创建多个 stream，并由不同的 task 消耗。
...
在此示例中，创建了一个名为 process_orders 的 task，并将其定义为在 task1 和 task2 成功完成后运行。这对于在 task 的有向无环图 (DAG) 中创建依赖关系非常有用。该 task 使用 'etl' 计算集群并将数据从暂存区域传输到数仓。
