...
| [ERROR_INTEGRATION](../16-notification/index.md) | 可选。用于 task 错误通知的通知集成名称，应用了特定的 [task 错误 payload ](./10-task-error-integration-payload.md)。                                                               |
...
- 多个从单个表 stream 使用更改数据的 task 检索不同的增量。当 task 使用 DML 语句使用 stream 中的更改数据时，stream 会提前偏移量。更改数据不再可供下一个 task 使用。目前，我们建议只有一个 task 使用 stream 中的更改数据。可以为同一表创建多个 stream，并由不同的 task 使用。
