...
CREATE TASK 语句用于定义一个新任务，该任务将按照预定的时间表或基于任务图执行指定的 SQL 语句。
...
| SUSPEND_TASK_AFTER_NUM_FAILURES                  | 可选。任务连续失败次数达到此值后将自动暂停。                                                                           |
...
- 当条件仅支持部分 `<boolean_expression>`
  ...
  - [STREAM_STATUS](../../../00-sql-reference/20-system-tables/system-stream-status.md) 支持在 SQL 表达式中进行评估。此函数指示指定流是否包含变更跟踪数据。您可以使用此函数在当前运行前评估指定流是否包含变更数据。如果结果为 FALSE，则任务不运行。
  - 布尔运算符如 AND、OR、NOT 等。
  - 数值、字符串和布尔类型之间的类型转换。
  - 比较运算符如等于、不等于、大于、小于等。

- 从单个表流消费变更数据的多个任务将获取不同的增量。当任务使用 DML 语句消费流中的变更数据时，流将推进偏移量。变更数据不再可供下一个任务消费。目前，我们建议仅由单个任务消费流中的变更数据。可以为同一表创建多个流，并由不同任务消费。
- 任务不会在每次执行时重试；每次执行都是串行的。每个脚本 SQL 逐一执行，没有并行执行。这确保了任务执行的顺序和依赖关系得到维护。
- 基于间隔的任务紧跟固定间隔点。这意味着如果当前任务执行时间超过间隔单位，下一个任务将立即执行。否则，下一个任务将等待下一个间隔单位触发。例如，如果任务定义为 1 秒间隔，一次任务执行耗时 1.5 秒，下一个任务将立即执行。如果一次任务执行耗时 0.5 秒，下一个任务将等待下一个 1 秒间隔开始。
...
在此示例中，创建了一个名为 my_daily_task 的任务。它使用 compute_wh 计算集群运行一个 SQL 语句，将数据从 source_table 插入到 summary_table 中。任务计划为每天太平洋时间上午 9 点运行。
...
此示例创建一个名为 mytask 的任务（如果不存在）。任务分配给 system 计算集群，计划每 2 分钟运行一次。如果连续失败三次，任务将暂停。任务执行 INSERT 操作到 compaction_test.test 表。
...
在此示例中，创建了一个名为 daily_sales_summary 的任务，具有秒级调度。计划每 30 秒运行一次。任务使用 'analytics' 计算集群，通过聚合 sales_data 表中的数据计算每日销售汇总。
...
在此示例中，创建了一个名为 process_orders 的任务，并定义为在 task1 和 task2 成功完成后运行。这对于创建任务的有向无环图（DAG）依赖关系很有用。任务使用 'etl' 计算集群，将数据从暂存区转移到数据仓库。
...
在此示例中，创建了一个名为 hourly_data_cleanup 的任务。它使用 maintenance 计算集群，计划每小时运行一次。任务删除 archived_data 表中超过 24 小时的数据。任务仅在 change_stream 流包含变更数据时运行。
...
在此示例中，创建了一个名为 mytask 的任务。它使用 mywh 计算集群，计划每 30 秒运行一次。任务执行一个包含 INSERT 语句和 DELETE 语句的 BEGIN 块。任务在两个语句执行后提交事务。当任务失败时，将触发名为 myerror 的错误集成。