...
| REMOVE AFTER | 从 task dag 中删除前置 task，如果未留下前置 task，则 task 将变为独立的 task 或根 task。 |
...
```
此示例更改 mytask task，将其计算集群更改为 new_warehouse，并将其计划更新为每天中午 UTC 运行。
...
```
在这里，mytask 执行的 SQL 语句已更改为将数据从 source_table 插入到 new_table 中。
...
```
在此示例中，我们正在向 MyTask1 task 添加依赖项。现在，它将在 'task2' 和 'task3' 成功完成后运行。这将在 task 的有向无环图 (DAG) 中创建依赖关系。

```sql
ALTER TASK MyTask1 REMOVE AFTER 'task2';
```
在这里，我们正在删除 MyTask1 task 的特定依赖项。它将不再在 'task2' 之后运行。如果要修改 task 在 task DAG 中的依赖项，这将非常有用。
```