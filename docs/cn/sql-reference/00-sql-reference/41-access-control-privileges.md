...
- 全局权限：这组权限包括适用于整个数据库管理系统的权限，而不是系统内的特定对象。全局权限授予影响数据库整体功能和管理的操作，例如创建或删除数据库、管理用户和角色以及修改系统级设置。关于包含哪些权限，请参见 [全局权限](#global-privileges)。
...
请注意，一旦您对数据库拥有以下任何权限或对数据库中的任何表拥有任何权限，您就可以使用 [USE DATABASE](../10-sql-commands/00-ddl/00-database/ddl-use-database.md) 命令来指定数据库。
...
| 权限 | 描述                                                              |
|:----------|:-------------------------------------------------------------------------|
| USAGE     | 可以使用UDF。例如，复制到阶段，预签名上传 |
| ALL       | 授予指定对象类型的READ, WRITE权限。             |

> 注意:
> 
> 1. 如果UDF已经被常量折叠，则不检查UDF授权。
> 2. 如果UDF是插入中的一个值，则不检查UDF授权。

## 目录权限

| 权限 | 描述                                              |
|:----------|:---------------------------------------------------------|
| SUPER     | 显示创建目录的语句。创建或删除目录。         |
| ALL       | 授予指定对象类型的所有权限。 |
