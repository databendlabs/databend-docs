...
:::note
- 从 Databend `v1.2.890-nightly` 开始，`TEXT` 可作为 `TSV` 的别名使用。
- 较旧版本的 Server 可能不支持 `TYPE = TEXT`，因此本页在语法和示例中仍以 `TSV` 为主，以兼容不同版本。
- 如果你的环境只面向 Databend `v1.2.890-nightly` 及之后版本，新的配置更推荐使用 `TYPE = TEXT`。
:::
...
Databend 中的 TSV（在 `v1.2.890-nightly` 及之后版本中称为 `TEXT`）在两种名称下使用相同的格式和选项。为了兼容旧版本 Server，本页仍以 `TSV` 作为主要术语。
