...
- 如果字符串来自序列化的 Array 或 Struct 字段，则会在 CSV 中加引号。
...
指定在查询结果中如何显示 "NaN" (Not-a-Number) 值。
...
3. TSV 可用于加载由以下方式生成的文件：
   ...
   2. MySQL `mysqldump` 命令，带有选项 `--tab`，不带 `--fields-enclosed-by` 或 `--fields-optinally-enclosed-by`，如果指定了后两者，请使用 CSV。
