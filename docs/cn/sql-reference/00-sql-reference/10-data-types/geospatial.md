...
- **GEOMETRY**: 使用平面坐标系（笛卡尔坐标），适用于 2D 几何对象。坐标表示为 (X, Y) 对，单位由相关的空间参考系统 (SRS) 确定。默认 SRID 为 0，但可以指定自定义 SRID。非常适合小规模测量，如城市或省级分析，它提供高计算速度和低资源使用率，但在较大区域上可能会引入显著误差。
...
:::note
GEOMETRY 和 GEOGRAPHY 类型目前是实验性功能。要使用这些类型创建表，请先执行 `SET enable_geo_create_table = 1` 启用它们。
:::
