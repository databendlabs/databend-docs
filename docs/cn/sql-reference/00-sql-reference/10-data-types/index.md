...
我们将来可能会改进该表达式，如果可能的话，可以将文字 `1` 解析为 String 值（concat 函数只接受 String 参数）。
...
- “id”列可以包含 NULL 值，因为它缺少“NOT NULL”约束。这意味着它可以存储整数或留空以表示缺少的数据。

- 由于“NOT NULL”约束，"name" 列必须始终具有值，不允许 NULL 值。

- 与 "id" 类似，"age" 列也可以保存 NULL 值，因为它没有 "NOT NULL" 约束，允许空条目或 NULL 来指示未知的年龄。
