...
MAP 数据结构用于保存一组 `Key:Value` 对，并使用 Array(Tuple(key, value)) 的嵌套数据结构来存储数据。它适用于数据类型固定但 `Key` 的值无法完全确定的情况。
...
在 Databend Map 中，对于某些数据类型（`Numeric`、`String`、`Timestamp` 和 `Date`）的值会创建布隆过滤器索引。
