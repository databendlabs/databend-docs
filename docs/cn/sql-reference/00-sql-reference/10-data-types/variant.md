...
VARIANT 类型包含表示为对象的键值对，其中每个键都是 VARCHAR，每个值都是 VARIANT。它的功能类似于其他编程语言中的“字典”、“哈希”或“映射”。可以使用**方括号**或**冒号**通过字段名访问值，也可以使用**点**，但仅适用于第二层及更深层（点不能用作第一层名称表示法，以避免与表和列之间的点表示法混淆）。
