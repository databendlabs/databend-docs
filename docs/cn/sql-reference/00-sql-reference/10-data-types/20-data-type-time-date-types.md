...
| TIMESTAMP | DATETIME | 8 字节     | 微秒        | 1000-01-01 00:00:00  | 9999-12-31 23:59:59.999999 UTC   | `YYYY-MM-DD hh:mm:ss[.fraction]`，最高可达微秒精度（6 位小数）      |
...
TIMESTAMP 值可以选择性地包含尾随的小数秒部分，最高可达微秒精度（6 位小数）。
...
例如，多伦多的夏令时于 2024 年 3 月 10 日凌晨 2:00 开始。因此，当天凌晨 2:00 至 3:00 之间的时间不存在。Databend 依赖 [Chrono](https://github.com/chronotope/chrono) 来确定每个时区的夏令时。如果提供了该范围内的时间，Databend 将返回错误：
...
| ----- | -------------------------------- | ------------------------------------------------------------------------------------------------------------------- |
|       |                                  | 日期说明符：                                                                                                          |
| %Y    | 2001                             | 完整的公历年份，零填充至4位。chrono支持从-262144到262143的年份。注意：公元前1年或公元9999年之后的年份，需要一个初始符号（+/-）。 |
| %C    | 20                               | 公历年份除以100，零填充至2位。                                                                                          |
| %y    | 01                               | 公历年份模100，零填充至2位。                                                                                            |
| %m    | 07                               | 月份编号（01–12），零填充至2位。                                                                                        |
| %b    | Jul                              | 缩写的月份名称。始终为3个字母。                                                                                         |
| %B    | July                             | 完整的月份名称。也接受相应的缩写在解析中。                                                                              |
| %h    | Jul                              | 与%b相同。                                                                                                             |
| %d    | 08                               | 日编号（01–31），零填充至2位。                                                                                          |
| %e    | 8                                | 与%d相同但空格填充。与%\_d相同。                                                                                        |
| %a    | Sun                              | 缩写的星期名称。始终为3个字母。                                                                                         |
| %A    | Sunday                           | 完整的星期名称。也接受相应的缩写在解析中。                                                                              |
| %w    | 0                                | 星期日 = 0，星期一 = 1，…，星期六 = 6。                                                                                  |
| %u    | 7                                | 星期一 = 1，星期二 = 2，…，星期日 = 7。（ISO 8601）                                                                      |
| %U    | 28                               | 以星期日开始的周编号（00–53），零填充至2位。                                                                            |
...
|       |                                  | 时间说明符：                                                                                                            |
| %H    | 00                               | 小时编号（00–23），零填充至2位。                                                                                        |
| %k    | 0                                | 与%H相同但空格填充。与%\_H相同。                                                                                        |
| %I    | 12                               | 12小时制的小时编号（01–12），零填充至2位。                                                                              |
| %l    | 12                               | 与%I相同但空格填充。与%\_I相同。                                                                                        |
| %P    | am                               | 12小时制的am或pm。                                                                                                     |
| %p    | AM                               | 12小时制的AM或PM。                                                                                                     |
| %M    | 34                               | 分钟编号（00–59），零填充至2位。                                                                                        |
| %S    | 60                               | 秒编号（00–60），零填充至2位。                                                                                          |
...
| %s    | 994518299                        | UNIX时间戳，自1970-01-01 00:00 UTC以来的秒数。Databend建议先将整数字符串转换为整数，而不是使用此说明符。有关示例，请参见[将整数转换为时间戳](/sql/sql-functions/datetime-functions/to-timestamp#example-2-converting-integer-to-timestamp)。 |
|       |                                  | 特殊说明符：                                                                                                            |
| %t    |                                  | 文字制表符（\t）。                                                                                                      |
| %n    |                                  | 文字换行符（\n）。                                                                                                      |
| %%    |                                  | 文字百分号。                                                                                                            |
...
- %f, %.f, %.3f, %.6f, %.9f, %3f, %6f, %9f:
  默认的 %f 是右对齐的，并且总是零填充到 9 位数字，以便与 glibc 和其他兼容，因此它总是计算自上一个整秒以来的纳秒数。例如，上一个整秒后的 7 毫秒将打印 007000000，解析 7000000 将得到相同的结果。
...
- %Z: 偏移量不会从解析的数据中填充，也不会进行验证。时区完全被忽略。类似于 glibc strptime 对这个格式代码的处理。
  ...
  例如，从缩写可靠地转换为偏移量是不可能的，例如 CDT 可以表示北美中部夏令时间或中国夏令时间。
...
- %+: 与 %Y-%m-%dT%H:%M:%S%.f%:z 相同，即秒的小数部分为 0、3、6 或 9 位，时区偏移中包含冒号。
  ...
  典型的 strftime 实现对这个说明符有不同的（且依赖于本地化的）格式。虽然 Chrono 的 %+ 格式更加稳定，但如果您想要控制确切的输出，最好避免使用这个说明符。

- %s: 这不进行填充，并且可以是负数。对于 Chrono 来说，它只考虑非闰秒，因此与 ISO C strftime 行为略有不同。