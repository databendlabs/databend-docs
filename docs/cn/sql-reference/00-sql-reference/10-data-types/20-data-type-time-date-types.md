...
|-------|----------------------------------|-----------------------------------------------------------------------------------------------------------------------|
|       |                                  | 日期说明符：                                                                                                          |
| %Y    | 2001                             | 完整的公历年份，零填充至4位。chrono支持从-262144到262143的年份。注意：公元前1年之前或公元9999年之后的年份，需要一个初始符号（+/-）。 |
...
- %Z: 偏移量不会从解析的数据中填充，也不会进行验证。时区完全被忽略。类似于 glibc strptime 处理此格式代码的方式。

  无法可靠地将缩写转换为偏移量，例如 CDT 可以表示北美中部夏令时间或中国夏令时间。

- %+: 与 %Y-%m-%dT%H:%M:%S%.f%:z 相同，即秒的小数部分为 0、3、6 或 9 位，时区偏移中包含冒号。

  此格式还支持在 %:z 的位置使用 Z 或 UTC。它们等同于 +00:00。

  请注意，所有 T、Z 和 UTC 都不区分大小写地解析。

  典型的 strftime 实现对此说明符有不同的（且与区域设置相关）格式。虽然 Chrono 的 %+ 格式更加稳定，但如果您想要控制确切的输出，最好避免使用此说明符。

- %s: 这不进行填充，并且可以是负数。对于 Chrono 而言，它仅考虑非闰秒，因此与 ISO C strftime 行为略有不同。