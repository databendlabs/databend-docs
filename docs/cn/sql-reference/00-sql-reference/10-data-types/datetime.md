...
| 名称      | 别名     | 存储大小 | 精度        | 最小值               | 最大值                          | 描述                                                                 |
| --------- | -------- | -------- | ----------- | -------------------- | ------------------------------- | -------------------------------------------------------------------- |
| DATE      |          | 4 字节    | 天          | 1000-01-01           | 9999-12-31                      | YYYY-MM-DD                                                           |
| TIMESTAMP | DATETIME | 8 字节    | 微秒        | 1000-01-01 00:00:00  | 9999-12-31 23:59:59.999999 UTC  | `YYYY-MM-DD hh:mm:ss[.fraction]`，最多支持微秒（6 位）精度          |
...
TIMESTAMP 值可以选择性地包含一个尾随的微秒部分，最多支持 6 位精度。
...
例如，多伦多的夏令时于 2024 年 3 月 10 日凌晨 2:00 开始。因此，当天凌晨 2:00 到 3:00 之间的时间不存在。Databend 依赖 [Chrono](https://github.com/chronotope/chrono) 来确定每个时区的夏令时。如果提供了该范围内的时间，Databend 将返回错误：
...
| %k    | 0                                | 与%H相同，但用空格填充。与%\_H相同。                                                                                                                                                                                                                                                                                      |
| %I    | 12                               | 12小时制的小时编号（01–12），零填充到2位数字。                                                                                                                                                                                                                                                                 |
| %l    | 12                               | 与%I相同，但用空格填充。与%\_I相同。                                                                                                                                                                                                                                                                                      |
| %P    | am                               | 12小时制的am或pm。                                                                                                                                                                                                                                                                                                     |
| %p    | AM                               | 12小时制的AM或PM。                                                                                                                                                                                                                                                                                                     |
...
| %.3f  | .026                             | 类似于.%f，但左对齐但固定为长度3。                                                                                                                                                                                                                                                                     |
| %.6f  | .026490                          | 类似于.%f，但左对齐但固定为长度6。                                                                                                                                                                                                                                                                     |
| %.9f  | .026490000                       | 类似于.%f，但左对齐但固定为长度9。                                                                                                                                                                                                                                                                     |
...
| %s    | 994518299                        | UNIX时间戳，自1970-01-01 00:00 UTC以来的秒数。Databend建议先将整数字符串转换为整数，而不是使用此说明符。有关示例，请参阅[将整数转换为时间戳](/sql/sql-functions/datetime-functions/to-timestamp#example-2-converting-integer-to-timestamp)。 |
|       |                                  | 特殊说明符：                                                                                                                                                                                                                                                                                                             |
| %t    |                                  | 文字制表符（\t）。                                                                                                                                                                                                                                                                                                               |
| %n    |                                  | 文字换行符（\n）。                                                                                                                                                                                                                                                                                                           |
| %%    |                                  | 文字百分号。                                                                                                                                                                                                                                                                                                           |
...
- %f, %.f, %.3f, %.6f, %.9f, %3f, %6f, %9f:
  默认的 %f 是右对齐的，并且为了与 glibc 和其他兼容性，总是填充到 9 位数字，因此它总是计算自上一个整秒以来的纳秒数。例如，在上一个整秒之后的 7 毫秒将打印 007000000，解析 7000000 将得到相同的结果。
  ...
  变体 %.3f、%.6f 和 %.9f 是左对齐的，并根据前面的数字打印 3、6 或 9 位小数。例如，在上一个整秒之后的 70 毫秒在 %.3f 下将打印 .070（注意：不是 .07），解析 .07、.070000 等将得到相同的结果。请注意，如果小数部分为零或下一个字符不是 .，它们可以读取为空，但会以指定的长度打印。
...
- %Z: 偏移量不会从解析的数据中填充，也不会被验证。时区完全被忽略。类似于 glibc strptime 对该格式代码的处理。

  无法可靠地从缩写转换为偏移量，例如 CDT 可以表示北美中部夏令时或中国夏令时。

- %+: 与 %Y-%m-%dT%H:%M:%S%.f%:z 相同，即秒的小数部分为 0、3、6 或 9 位，时区偏移中带有冒号。

  此格式还支持在 %:z 的位置使用 Z 或 UTC。它们等同于 +00:00。

  请注意，所有 T、Z 和 UTC 都不区分大小写地解析。

  典型的 strftime 实现在此说明符上有不同的（且依赖于区域设置的）格式。虽然 Chrono 的 %+ 格式更加稳定，但如果您想要控制确切的输出，最好避免使用此说明符。
