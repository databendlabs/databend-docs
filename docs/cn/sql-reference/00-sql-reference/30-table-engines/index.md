...
| 引擎 | 最适合 | 亮点 |
| ------ | -------- | ---------- |
| [Fuse 引擎表](table-engines/fuse) | Databend 原生表 | 基于快照的存储、自动聚类、变更跟踪 |
| [Apache Iceberg™ 表](table-engines/iceberg) | Lakehouse 目录 | 时间回溯（Time Travel）、模式演进、REST/Hive/存储目录 |
| [Apache Hive 表](table-engines/hive) | Hive 元存储数据 | 通过外部表查询 Hive 管理的数据存储 |
| [Delta Lake 引擎](table-engines/delta) | Delta Lake 数据集 | 在对象存储中读取具备 ACID 保证的 Delta 表 |
