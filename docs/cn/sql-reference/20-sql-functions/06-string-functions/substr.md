...
从给定字符串的特定位置提取包含特定数量字符的字符串。

- 没有 `len` 参数的形式从字符串 `str` 的 `pos` 位置开始返回子字符串。
- 带有 `len` 参数的形式从字符串 `str` 的 `pos` 位置开始返回长度为 `len` 的子字符串。

`pos` 也可以使用负值。在这种情况下，子字符串的开头是距离字符串结尾 pos 个字符，而不是开头。在任何形式的函数中，`pos` 都可以使用负值。`pos` 值为 0 时返回空字符串。要提取子字符串的字符串中第一个字符的位置被认为是 1。
...
|-----------|--------------------------------------------------------------------------------------------------|
| `<str>`   | 要从中提取字符的主字符串                                                                                     |
| `<pos>`   | 子字符串开始的位置（从 1 开始）。如果是负数，则从末尾开始计数                                                                       |
| `<len>`   | 要提取的子字符串的最大长度                                                                                       |
