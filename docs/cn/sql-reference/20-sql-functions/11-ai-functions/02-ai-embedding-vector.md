...
该函数接受文本输入，并返回一个高维向量，该向量表示输入文本的语义意义和上下文。embeddings 是使用大型文本语料库上的预训练模型创建的，捕获了连续空间中单词和短语之间的关系。
...
embeddings 作为 `FLOAT32` 值的数组存储在 embedding 列中，该列具有 `ARRAY(FLOAT32 NOT NULL)` 列类型。
...
上面的查询显示，对于每个文档，生成的 embeddings 的长度为 1536（维度）。
