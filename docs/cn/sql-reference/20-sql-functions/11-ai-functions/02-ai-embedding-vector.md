...
该函数接受文本输入，并返回一个高维向量，该向量表示输入文本的语义含义和上下文。这些 embeddings 是使用大型文本语料库上的预训练模型创建的，捕获了连续空间中单词和短语之间的关系。
...
这些 embeddings 作为 `FLOAT32` 值的数组存储在 embedding 列中，该列具有 `ARRAY(FLOAT32 NOT NULL)` 列类型。
