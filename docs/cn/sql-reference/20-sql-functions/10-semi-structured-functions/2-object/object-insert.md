...
| `<key>`             | 要插入或更新的键。                                                                                                                                                                                                                                   |   |
| `<value>`           | 要赋给键的值。                                                                                                                                                                                                                                       |   |
| `<update_flag>` | 布尔标志，控制当指定键已存在于 JSON 对象中时是否替换其值。若为 `true` 则替换已有键的值；若为 `false`（或省略）则键存在时报错。 |   |
...
此示例展示如何通过设置更新标志为 `true`，将现有键 'a' 的值从 1 更新为 10：
...
此示例演示未设置更新标志为 `true` 时，尝试为现有键 'a' 插入值会报错：
