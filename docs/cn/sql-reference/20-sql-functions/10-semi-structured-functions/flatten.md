...
| PATH                | 指定要展平的输入数据中数组或对象的路径。                                                                                                                                                                                 | -       |
| OUTER               | 如果设置为 TRUE，则结果为零的行仍将包含在输出中，但这些行的 KEY、INDEX 和 VALUE 列中的值将设置为 NULL。                                                                                                                   | FALSE   |
| RECURSIVE           | 如果设置为 TRUE，该函数将继续展平嵌套元素。                                                                                                                                                                               | FALSE   |
| MODE                | 控制是仅展平对象 ('OBJECT')、仅展平数组 ('ARRAY') 还是两者都展平 ('BOTH')。                                                                                                                                                  | 'BOTH'  |
| LATERAL             | LATERAL 是一个可选关键字，用于引用在 FROM 子句中 LATERAL 关键字左侧定义的列。LATERAL 启用前一个表表达式和函数之间的交叉引用。                                                                                             | -       |
...
:::note
将 LATERAL 关键字与 FLATTEN 一起使用时，可能无法显式提供这些输出列，因为 LATERAL 引入了动态交叉引用，从而改变了输出结构。
:::

| Column | Description                                                                              |
|--------|------------------------------------------------------------------------------------------|
| SEQ    | 与输入关联的唯一序列号。                                                                 |
| KEY    | 展开值的键。如果展平的元素不包含键，则将其设置为 NULL。                                       |
| PATH   | 展平元素的路径。                                                                         |
| INDEX  | 如果元素是数组，则此列包含其索引；否则，将其设置为 NULL。                                   |
| VALUE  | 展平元素的值。                                                                         |
| THIS   | 此列标识当前正在展平的元素。                                                               |
...
本示例演示了 FLATTEN 函数关于 PATH、OUTER、RECURSIVE 和 MODE 参数的行为。
...
-- PATH 有助于从原始 JSON 数据中选择特定路径上的元素。
SELECT
  *
FROM
  FLATTEN (
...
-- OUTER 确定输出中是否包含零行扩展。
-- 在第一个示例中，OUTER => TRUE 与一个空 JSON 数组一起使用，这将导致零行扩展。
-- 即使没有要展平的值，行也会包含在输出中。
SELECT
  *
FROM
...
-- 在第二个示例中，省略了 OUTER，输出显示了当未指定 OUTER 时，如何不包含具有零结果的行。
SELECT
  *
FROM
...
-- 使用 LATERAL FLATTEN 显示包含多个商品的 Tim Hortons 交易
SELECT
    t.transaction_id,
    t.customer_id,
