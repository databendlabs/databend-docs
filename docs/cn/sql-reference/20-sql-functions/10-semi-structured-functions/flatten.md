...
| 参数 / 关键字 | 描述                                                                                                                                                                                                             | 默认值 |
|---------------------|-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|---------|
| INPUT               | 指定要展开的 JSON 或数组数据。                                                                                                                                                                            | -       |
| PATH                | 指定要展开的输入数据中数组或对象的路径。                                                                                                                                                                            | -       |
| OUTER               | 如果设置为 TRUE，则结果为零的行仍将包含在输出中，但这些行的 KEY、INDEX 和 VALUE 列中的值将设置为 NULL。                                                     | FALSE   |
| RECURSIVE           | 如果设置为 TRUE，该函数将继续展开嵌套元素。                                                                                                                                                  | FALSE   |
| MODE                | 控制是仅展开对象 ('OBJECT')、仅展开数组 ('ARRAY') 还是两者都展开 ('BOTH')。                                                                                                                           | 'BOTH'  |
...
:::note
将 LATERAL 关键字与 FLATTEN 一起使用时，可能不会显式提供这些输出列，因为 LATERAL 引入了动态交叉引用，从而改变了输出结构。
:::

| 列 | 描述                                                                              |
|--------|------------------------------------------------------------------------------------------|
| SEQ    | 与输入关联的唯一序列号。                                      |
| KEY    | 展开值的键。如果展开的元素不包含键，则将其设置为 NULL。|
| PATH   | 展开元素的路径。                                                           |
...
-- 在此示例中，使用了 MODE => 'ARRAY'，这意味着只有 JSON 数据中的数组会被展平。
SELECT
  *
FROM
  FLATTEN (
