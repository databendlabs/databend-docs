...
检查指定的 JSON 路径表达式是否与 JSON 数据中的某些条件匹配。请注意，`@@` 操作符是此函数的同义词。更多信息，请参阅 [JSON 操作符](../../../10-sql-commands/30-query-operators/index.md)。
...
- `json_path_expression`：指定要在 JSON 数据中检查的条件。此表达式描述了要匹配的特定路径或条件，例如验证 JSON 结构中特定字段值是否满足某些条件。`$` 符号代表 JSON 数据的根节点。它用于起始路径表达式，并表示 JSON 结构中的顶层对象。
...
-- 检查 JSON 路径 $.b 数组中从第二个元素到最后一个元素是否都大于或等于 2
