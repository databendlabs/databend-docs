...
JQ 函数是一个返回集合的 SQL 函数，允许你将 [jq](https://jqlang.github.io/jq/) 过滤器应用于存储在 Variant 列中的 JSON 数据。通过此函数，你可以应用指定的 jq 过滤器来处理 JSON 数据，并将结果作为行集返回。
...
JQ 函数返回一个 JSON 值集合，其中每个值对应于基于 `<jq_expression>` 转换或提取的结果中的一个元素。
