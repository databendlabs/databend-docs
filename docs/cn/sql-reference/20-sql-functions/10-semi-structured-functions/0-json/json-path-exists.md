...
- json_data: 指定要搜索的 JSON 数据。它可以是 JSON 对象（JSON object）或数组（array）。

- json_path_expression: 指定要在 JSON 数据中检查的路径，该路径以 `$` 表示的 JSON 数据根（root）开始。你还可以在表达式中包含条件，使用 `@` 引用当前正在评估的节点（current node）或元素，以筛选结果。
