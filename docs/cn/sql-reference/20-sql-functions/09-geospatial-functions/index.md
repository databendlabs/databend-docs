...
Databend 内置两套互补的地理空间能力：PostGIS 风格的几何函数（PostGIS-style Geometry Functions）用于构建与分析形状，以及 H3 工具（H3 Utilities）用于全球六边形索引。下表按任务对函数分组，方便快速定位所需工具，布局与 Snowflake 文档类似。

## 几何构造函数

| 函数 | 描述 | 示例 |
|----------|-------------|---------|
| [ST_MAKEGEOMPOINT](st-makegeompoint.md) / [ST_GEOM_POINT](st-geom-point.md) | 构造 Point 几何体 | `ST_MAKEGEOMPOINT(-122.35, 37.55)` → `POINT(-122.35 37.55)` |
...
| 函数 | 描述 | 示例 |
|----------|-------------|---------|
| [ST_GEOMETRYFROMTEXT](st-geometryfromtext.md) / [ST_GEOMFROMTEXT](st-geomfromtext.md) | 将 WKT 转为几何体 | `ST_GEOMETRYFROMTEXT('POINT(-122.35 37.55)')` → `POINT(-122.35 37.55)` |
| [ST_GEOMETRYFROMWKB](st-geometryfromwkb.md) / [ST_GEOMFROMWKB](st-geomfromwkb.md) | 将 WKB 转为几何体 | `ST_GEOMETRYFROMWKB(...)` → `POINT(...)` |
| [ST_GEOMETRYFROMEWKT](st-geometryfromewkt.md) / [ST_GEOMFROMEWKT](st-geomfromewkt.md) | 将 EWKT 转为几何体 | `ST_GEOMETRYFROMEWKT('SRID=4326;POINT(-122.35 37.55)')` → `POINT(-122.35 37.55)` |
| [ST_GEOMETRYFROMEWKB](st-geometryfromewkb.md) / [ST_GEOMFROMEWKB](st-geomfromewkb.md) | 将 EWKB 转为几何体 | `ST_GEOMETRYFROMEWKB(...)` → `POINT(...)` |
| [ST_GEOMFROMGEOHASH](st-geomfromgeohash.md) | 将 GeoHash 转为几何体 | `ST_GEOMFROMGEOHASH('9q8yyk8')` → `POLYGON(...)` |
| [ST_GEOMPOINTFROMGEOHASH](st-geompointfromgeohash.md) | 将 GeoHash 转为 Point 几何体 | `ST_GEOMPOINTFROMGEOHASH('9q8yyk8')` → `POINT(...)` |
| [TO_GEOMETRY](to-geometry.md) | 将多种格式解析为几何体 | `TO_GEOMETRY('POINT(-122.35 37.55)')` → `POINT(-122.35 37.55)` |

## 几何输出

| 函数 | 描述 | 示例 |
|----------|-------------|---------|
| [ST_ASTEXT](st-astext.md) | 将几何体转为 WKT | `ST_ASTEXT(ST_MAKEGEOMPOINT(-122.35, 37.55))` → `'POINT(-122.35 37.55)'` |
| [ST_ASWKT](st-aswkt.md) | 将几何体转为 WKT | `ST_ASWKT(ST_MAKEGEOMPOINT(-122.35, 37.55))` → `'POINT(-122.35 37.55)'` |
| [ST_ASBINARY](st-asbinary.md) / [ST_ASWKB](st-aswkb.md) | 将几何体转为 WKB | `ST_ASBINARY(ST_MAKEGEOMPOINT(-122.35, 37.55))` → `WKB representation` |
| [ST_ASEWKT](st-asewkt.md) | 将几何体转为 EWKT | `ST_ASEWKT(ST_MAKEGEOMPOINT(-122.35, 37.55))` → `'SRID=4326;POINT(-122.35 37.55)'` |
| [ST_ASEWKB](st-asewkb.md) | 将几何体转为 EWKB | `ST_ASEWKB(ST_MAKEGEOMPOINT(-122.35, 37.55))` → `EWKB representation` |
| [ST_ASGEOJSON](st-asgeojson.md) | 将几何体转为 GeoJSON | `ST_ASGEOJSON(ST_MAKEGEOMPOINT(-122.35, 37.55))` → `'{"type":"Point","coordinates":[-122.35,37.55]}'` |
| [ST_GEOHASH](st-geohash.md) | 将几何体转为 GeoHash | `ST_GEOHASH(ST_MAKEGEOMPOINT(-122.35, 37.55), 7)` → `'9q8yyk8'` |
| [TO_STRING](to-string.md) | 将几何体转为字符串 | `TO_STRING(ST_MAKEGEOMPOINT(-122.35, 37.55))` → `'POINT(-122.35 37.55)'` |
...
| [ST_POINTN](st-pointn.md) | 从 LineString 返回指定点 | `ST_POINTN(ST_MAKELINE(...), 1)` → `POINT(-122.35 37.55)` |
| [ST_STARTPOINT](st-startpoint.md) | 返回 LineString 的第一个点 | `ST_STARTPOINT(ST_MAKELINE(...))` → `POINT(-122.35 37.55)` |
| [ST_ENDPOINT](st-endpoint.md) | 返回 LineString 的最后一个点 | `ST_ENDPOINT(ST_MAKELINE(...))` → `POINT(-122.40 37.60)` |
...
| [POINT_IN_POLYGON](point-in-polygon.md) | 检查点是否在多边形内 | `POINT_IN_POLYGON([lon, lat], [[p1_lon, p1_lat], ...])` → `TRUE` |
...
| 函数 | 描述 | 示例 |
|----------|-------------|---------|
| [GEO_TO_H3](geo-to-h3.md) | 将经度/纬度转为 H3 索引 | `GEO_TO_H3(37.7950, 55.7129, 15)` → `644325524701193974` |
| [H3_TO_GEO](h3-to-geo.md) | 将 H3 索引转为经度/纬度 | `H3_TO_GEO(644325524701193974)` → `[37.7950, 55.7129]` |
| [H3_TO_STRING](h3-to-string.md) | 将 H3 索引转为字符串形式 | `H3_TO_STRING(644325524701193974)` → `'8f2830828052d25'` |
| [STRING_TO_H3](string-to-h3.md) | 将 H3 字符串转为索引 | `STRING_TO_H3('8f2830828052d25')` → `644325524701193974` |
...
| [H3_HEX_AREA_KM2](h3-hex-area-km2.md) | 返回平均六边形面积（平方公里） | `H3_HEX_AREA_KM2(10)` → `0.0152` |
...
| [H3_NUM_HEXAGONS](h3-num-hexagons.md) | 返回某分辨率下的六边形数量 | `H3_NUM_HEXAGONS(2)` → `5882` |

## H3 邻域

| 函数 | 描述 | 示例 |
|----------|-------------|---------|
| [H3_DISTANCE](h3-distance.md) | 返回两索引间的网格距离 | `H3_DISTANCE(599119489002373119, 599119491149856767)` → `1` |
| [H3_INDEXES_ARE_NEIGHBORS](h3-indexes-are-neighbors.md) | 测试两索引是否相邻 | `H3_INDEXES_ARE_NEIGHBORS(599119489002373119, 599119491149856767)` → `TRUE` |
...
| 函数 | 描述 | 示例 |
|----------|-------------|---------|
| [H3_GET_UNIDIRECTIONAL_EDGE](h3-get-unidirectional-edge.md) | 返回两相邻单元格之间的边 | `H3_GET_UNIDIRECTIONAL_EDGE(from_h3, to_h3)` → `edge_index` |
...
| [H3_GET_UNIDIRECTIONAL_EDGES_FROM_HEXAGON](h3-get-unidirectional-edges-from-hexagon.md) | 列出从某单元格出发的所有边 | `H3_GET_UNIDIRECTIONAL_EDGES_FROM_HEXAGON(h3_index)` → `[edge1, edge2, ...]` |
...
| 函数 | 描述 | 示例 |
|----------|-------------|---------|
| [H3_EDGE_LENGTH_KM](h3-edge-length-km.md) | 返回平均边长（公里） | `H3_EDGE_LENGTH_KM(10)` → `0.065` |
| [H3_EDGE_LENGTH_M](h3-edge-length-m.md) | 返回平均边长（米） | `H3_EDGE_LENGTH_M(10)` → `65.91` |
| [H3_EXACT_EDGE_LENGTH_KM](h3-exact-edge-length-km.md) | 返回精确边长（公里） | `H3_EXACT_EDGE_LENGTH_KM(edge_index)` → `0.066` |
...
| [H3_EDGE_ANGLE](h3-edge-angle.md) | 返回两边之间的夹角（弧度） | `H3_EDGE_ANGLE(edge1, edge2)` → `1.047` |