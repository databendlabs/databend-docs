...
使用 [lambda 表达式](../../00-sql-reference/42-lambda-expressions.md) 将转换应用于 map 中的每个 key。
...
返回一个 map，其值与输入 map 相同，但 key 根据指定的 lambda 转换进行修改。

## 示例

此示例将 1,000 添加到每个产品 ID，创建一个新的 map，其中更新了 key，同时保持关联的价格不变：
