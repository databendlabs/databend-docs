...
- `enable_expand_roles=1` (默认):
    ...
    - SHOW_GRANTS 递归地展开继承的权限，这意味着如果一个角色被授予了另一个角色，它将显示所有继承的权限。
...
如果将角色 `b` 授予角色 `a` 并再次检查角色 `a` 上的授权，您可以看到 `t2` 上的 `SELECT` 权限现在包含在角色 `a` 中：
...
如果将 `enable_expand_roles` 设置为 `0` 并再次检查角色 `a` 上的授权，则结果将显示 `GRANT ROLE` 语句，而不是列出从角色 `b` 继承的特定权限：
