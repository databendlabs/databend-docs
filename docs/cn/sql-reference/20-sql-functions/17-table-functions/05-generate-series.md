...
从指定点开始生成数据集，到另一个指定点结束，并可选择使用递增值。GENERATE_SERIES 函数适用于以下数据类型：
...
| step_interval 	| 步长间隔，确定序列中相邻值之间的差值。对于整数序列，默认值为 1。对于日期序列，默认步长间隔为 1 天。对于时间戳序列，默认步长间隔为 1 微秒。                                                                                                                                                               	|


:::note
在处理像 GENERATE_SERIES 和 RANGE 这样的函数时，一个关键的区别在于它们的边界特性。GENERATE_SERIES 受左右两边的约束，而 RANGE 仅受左侧的约束。例如，使用 RANGE(1, 11) 等同于 GENERATE_SERIES(1, 10)。
:::

## 返回类型

返回一个列表，其中包含从 *start* 到 *stop* 的连续数值、日期或时间戳序列。
...
此示例使用 GENERATE_SERIES 函数和左连接运算符来处理由于特定范围内缺少信息而导致的查询结果中的空白。
...
要关闭 2020 年 1 月 1 日至 1 月 10 日之间的空白，请使用以下查询：
