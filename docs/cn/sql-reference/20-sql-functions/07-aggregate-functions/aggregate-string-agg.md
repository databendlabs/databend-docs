...
STRING_AGG() 函数（也称为 GROUP_CONCAT 或 LISTAGG）将列的所有非 NULL 值连接到一个字符串，并用分隔符分隔。
...
:::info
如果 `<expr>` 不是字符串表达式，则应使用 `::VARCHAR` 进行转换。
...
| 参数        | 描述                                                                  |
|-------------|-----------------------------------------------------------------------|
| `<expr>`    | 任何字符串表达式（如果不是字符串，则使用 `::VARCHAR` 进行转换）                  |
...
| WITHIN GROUP [&lt;orderby_clause&gt;](https://docs.databend.com/sql/sql-commands/query-syntax/query-select#order-by-clause) | 定义有序集聚合中值的顺序                                                     |
...
**查询演示：使用 `WITHIN GROUP` 连接带有分隔符的编程语言名称**
```sql
