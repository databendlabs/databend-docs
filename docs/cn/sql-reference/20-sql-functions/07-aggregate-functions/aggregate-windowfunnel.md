...
-   如果在窗口中按顺序发生链中的事件，则计数器会递增。如果事件的顺序被打乱，则计数器不会递增。
...
找出用户 `user_id` 在一小时窗口滑动中可以通过链走多远。
...
* 用户 `100126` 级别为 2 (`login -> visit`)。
* 用户 `100125` 级别为 3 (`login -> visit -> cart`)。
* 用户 `100123` 级别为 4 (`login -> visit -> cart -> purchase`)。