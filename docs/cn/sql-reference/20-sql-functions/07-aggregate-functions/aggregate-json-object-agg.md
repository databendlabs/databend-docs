...
将键值对转换为 JSON 对象。对于输入的每一行，它都会生成一个键值对，其中键派生自 `<key_expression>`，值派生自 `<value_expression>`。然后，这些键值对被组合成一个 JSON 对象。
...
此示例演示了如何使用 JSON_OBJECT_AGG 将不同类型的数据（如 decimals、integers、JSON variants 和 arrays）聚合到 JSON 对象中，其中列 b 作为每个 JSON 对象的键：
