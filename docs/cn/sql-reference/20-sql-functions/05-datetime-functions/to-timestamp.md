...
- 如果转换失败，将返回错误。 要避免此类错误，可以使用 [TRY_TO_TIMESTAMP](try-to-timestamp.md) 函数。
...
:::note
- 您也可以使用 `SELECT TO_TIMESTAMP('994518299', '%s')` 进行转换，但不建议这样做。 对于此类转换，Databend 建议使用上面示例中的方法以获得更好的性能。

- 时间戳值的范围为 1000-01-01 00:00:00.000000 到 9999-12-31 23:59:59.999999。 如果您运行以下语句，Databend 将返回错误：
