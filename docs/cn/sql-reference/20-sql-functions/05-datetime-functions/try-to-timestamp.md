...
`TRY_TO_TIMESTAMP` 是 Databend 中 [TO_TIMESTAMP](to-timestamp.md) 的变体，它在将输入表达式转换为时间戳的同时，还通过在转换失败时返回 NULL 而不是引发错误来包含错误处理支持。
...
如果给定两个参数，该函数会根据第二个字符串中指定的模式将第一个字符串转换为时间戳。 要指定模式，请使用说明符。 说明符允许您定义日期和时间值的所需格式。 有关支持的说明符的完整列表，请参见 [格式化日期和时间](../../00-sql-reference/10-data-types/datetime.md#formatting-date-and-time)。
