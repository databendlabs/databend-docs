...
*date1* 和 *date2* 可以是 DATE 类型、TIMESTAMP 类型或两者的混合类型。
...
- 如果 *date1* 和 *date2* 落在各自月份的同一天，或者都是各自月份的最后一天，则结果为整数。否则，该函数基于 31 天的月份计算结果的小数部分。
...
- 如果 *date1* 和 *date2* 是同一天，则该函数会忽略任何时间分量并返回 0。
