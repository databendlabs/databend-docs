...
返回窗口帧中第 `N` 个位置的值，其中 `N` 是一个指定的整数，用于确定值的确切位置。
...
- `[ { IGNORE | RESPECT } NULLS ]`: 控制如何在窗口函数中处理 NULL 值。
  ...
  - 当设置为 `IGNORE NULLS` 时，NULL 值将被排除在考虑之外，并且该函数仅对非 NULL 值进行操作。
  - 如果窗口帧中的所有值均为 NULL，则即使指定了 `IGNORE NULLS`，该函数也会返回 NULL。

- 有关窗口帧的语法，请参见 [Window Frame Syntax](index.md#window-frame-syntax)。
...
此示例使用 `IGNORE NULLS` 选项从窗口帧中排除 NULL 值：
