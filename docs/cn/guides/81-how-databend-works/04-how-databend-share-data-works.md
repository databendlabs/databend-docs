...
不同团队需要同一数据的不同部分。传统解决方案需要多次复制数据，这不仅成本高昂，而且难以维护。

Databend 的 **[ATTACH TABLE](/sql/sql-commands/ddl/table/attach-table)** 优雅地解决了这一问题：它能在不复制数据的情况下，为同一数据创建多个“视图”。这得益于 Databend 的**真正计算存储分离**架构——无论使用云存储还是本地对象存储，都能实现**一次存储，随处访问**。

可以将 ATTACH TABLE 类比为计算机快捷方式：它们指向原始文件，而无需复制文件。
...
**列级安全性**：团队只能看到所需内容——营销团队无法查看利润，财务团队无法查看客户邮箱

**强一致性**：永远不会读取部分更新，始终看到完整快照——非常适合财务报告和合规要求

**完整性能**：所有索引自动工作，性能与常规表相同
...
多个 Databend 集群可以同时执行此流程而无需协调——这体现了真正的计算存储分离。

ATTACH TABLE 代表了一个根本性转变：**从为每个用例复制数据转向一个副本多个视图**。无论是在云环境还是本地环境中，Databend 的架构都能实现强大而高效的数据共享，同时确保企业级一致性和安全性。