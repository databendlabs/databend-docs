...
Databend 的查询优化器通过四个不同阶段将 SQL 查询转换为高效执行计划：
...
**功能：** 在可能时，用表统计信息中的常量值替换特定聚合函数（MIN/MAX），避免全表扫描。
...
**功能：** 从存储层收集实际表/列统计信息，附加到扫描节点。需要时通过添加随机过滤器处理行级采样。
...
**功能：** 将过滤条件从下层节点上拉到计划树顶层，支持更全面的过滤器优化。对于内连接，将连接条件拉入过滤器并转换为带过滤器的交叉连接。

**6. 默认重写规则（RecursiveRuleOptimizer）**

**功能：** 递归应用转换规则集到查询计划。每个规则匹配计划中的特定模式并转换为更高效形式，直到无法继续转换。
...
**功能：** 将过滤器推送到存储层，使 Databend 能跳过读取无关数据块。
...
**功能：** 将 LIMIT 子句下推到计划中，减少昂贵操作处理的数据量。
...
**功能：** 将聚合操作拆分为 Partial/Final 两阶段，支持分布式执行。Partial 阶段在各节点本地执行，Final 阶段合并结果，这是并行聚合处理的前提。
...
**功能：** 基于表统计和连接条件，使用动态规划寻找最优连接顺序：
1. 构建表示表间连接关系的查询图
2. 使用动态规划算法（DPhyp）枚举所有可能连接顺序
3. 对多表复杂查询自适应切换至贪心算法
4. 基于表基数和选择性估算各连接顺序成本
5. 选择估算成本最低的连接顺序

对多连接查询至关重要，连接顺序显著影响查询性能。
...
**功能：** 当优化器判定安全时（通过 `single_to_inner` 标记），将 LeftSingle/RightSingle 连接类型转换为更高效的内连接，不改变查询语义。
...
**功能：** 使用并查集算法识别并移除冗余连接条件（特别是传递性隐含条件）：
1. 初始为每列创建独立等价组
2. 处理连接条件时合并相等列的等价组
3. 跳过已在同组的列条件
4. 保留维持查询语义所需的最简连接条件集

减少执行时需评估的连接条件数量，简化连接操作并提升性能。
...
3. 相应调整连接条件和类型（如 LEFT 转 RIGHT）

因 Databend 通常在哈希连接中以右侧作为构建端，此优化确保小表用于构建哈希表，通过减少内存占用和构建时间提升连接性能。
...
**注意：** 这些值反映不同操作的相对成本（网络操作 50 > 哈希表 10 > 聚合 5 > 计算 1）。成本因子与基数估算结合计算各操作总成本，优化器选择总成本最低的实现方案。成本计算包含所有子操作。

## 总结

Databend 查询优化器通过多阶段流水线将 SQL 查询转换为高效物理执行计划，核心机制包括：
1. **准备阶段：** 子查询去相关与统计信息收集
2. **逻辑优化：** 应用基于规则的转换（如过滤器下推、聚合规范化）
