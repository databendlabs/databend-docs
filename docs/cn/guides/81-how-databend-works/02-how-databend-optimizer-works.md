...
Databend 的查询优化器基于几个关键抽象构建，这些抽象协同工作将 SQL 查询转换为高效的执行计划：
...
Databend 的查询优化器遵循精心设计的管道将 SQL 查询转换为高效执行计划：
...
**作用：** 在可能时用表统计信息中的常量值替换聚合函数（MIN/MAX），避免全表扫描。
...
**作用：** 将过滤条件上拉至计划树顶部，支持更全面的优化。对于内连接，将连接条件并入过滤器并转换为带过滤的交叉连接。
...
**作用：** 将聚合拆分为部分（Partial）和最终（Final）两阶段，支持分布式执行。部分聚合在节点本地执行，最终聚合合并结果，为并行处理提供基础。
...
**作用：** 基于表统计和连接条件，使用动态规划（DPhyp）寻找最优连接顺序：
1. 构建表间连接关系图
2. 枚举所有可能连接顺序
3. 对多表查询自适应切换至贪心算法
4. 基于基数和选择性估算成本
...
**作用：** 当优化器标记 `single_to_inner` 且语义安全时，将单连接类型转换为更高效的内连接。
...
1. 比较左右输入基数
2. 内连接中左侧行少时交换输入
3. 调整连接类型（如 LEFT 转 RIGHT）
4. 确保小表用于哈希连接构建端，减少内存和构建时间
...
**注：** 数值表示操作相对成本（如网络操作成本 50 >> 计算成本 1）。总成本通过基数估算递归累加，优化器选择最低成本方案。

## 总结

Databend 的查询优化器通过多阶段管道将 SQL 查询转换为高效物理执行计划，核心包括：
- **SExpr** 计划表示
- 丰富转换规则集
- 详细统计信息
- 成本模型
...
3.  **连接优化：** 动态规划确定最佳连接顺序
4.  **物理规划：** Cascades 框架选择最优物理算子（如哈希连接 vs 嵌套循环连接）
