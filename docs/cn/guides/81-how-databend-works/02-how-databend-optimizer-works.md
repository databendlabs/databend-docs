...
Databend 的查询优化器通过四个不同阶段将 SQL 查询转换为高效的执行计划。让我们分析每个阶段及其组件优化器：
...
**优化后：**
```
# 使用统计信息中的预计算值替代 MIN 聚合
EvalScalar (price_min)
└─ DummyTableScan
```

**作用：** 当可能时，用表统计信息中的常量值替代聚合函数 (MIN, MAX)，避免全表扫描。
...
**作用：** 将过滤条件从下层节点上提到计划树顶部，实现更全面的过滤优化。对于内连接，还将连接条件纳入过滤器，转换为带过滤条件的交叉连接。

**6. 默认重写规则 (RecursiveRuleOptimizer)**

**作用：** 递归应用一组转换规则到查询计划。每条规则匹配计划中的特定模式并转换为更高效的形式。优化器持续应用规则直到无法继续转换。
...
**作用：** 将过滤器下推到存储层，使 Databend 能够跳过读取无关数据块。
...
**作用：** 使用动态规划算法基于表统计信息和连接条件寻找最优连接顺序。优化器：

1. 构建表示表间连接关系的查询图
2. 使用动态规划算法 (DPhyp - 动态规划超图) 枚举所有可能的连接顺序
3. 对包含多表的复杂查询自适应切换为贪心算法
4. 基于表基数和选择性估算每个连接顺序的成本
5. 选择估算成本最低的连接顺序

该优化器对涉及多表连接的查询尤为重要，连接顺序会显著影响查询性能。
...
**作用：** 使用并查集算法识别并移除冗余连接条件，特别是那些由其他条件传递隐含的条件。该优化器：

1. 初始为每列分配自己的等价组
2. 处理每个连接条件，合并相等列的等价组
3. 当两列已在同一等价组时跳过条件
4. 保留维持相同查询语义所需的最少连接条件集

此优化减少查询执行期间需要评估的连接条件数量，简化连接操作并可能提升性能。
...
**注意:** 这些数值展示了不同操作的相对成本。例如网络传输 (50) 比简单计算 (1) 昂贵得多，而哈希表操作 (10) 比聚合操作 (5) 成本更高。

这些成本因子会与基数 (行数) 估算值结合，计算每个操作的总成本。优化器随后会选择总成本最低的实现方案。
...
优化流程包含：
1. **准备阶段:** 解关联子查询并收集必要统计信息
2. **逻辑优化:** 应用基于规则的转换 (如谓词下推、聚合规范化) 来优化逻辑计划结构
3. **连接优化:** 使用动态规划等技术策略性地确定最佳连接顺序和方法
4. **物理计划生成:** 基于 Cascades 框架选择最具成本效益的物理算子 (如 Hash Join 与 Nested Loop Join)

通过系统性地应用这些步骤，优化器旨在最小化资源消耗 (CPU、内存、I/O) 并最大化查询执行速度。