...
- [变体数据类型](/sql/sql-reference/data-types/variant)
- [半结构化函数](/sql/sql-functions/semi-structured-functions/)

## 核心概念

Databend 的变体类型（Variant）是专为处理 JSON 等半结构化数据设计的灵活数据类型。它提供与 Snowflake 兼容的语法和函数，同时通过高效存储格式和优化访问机制实现高性能。Databend 中大多数变体相关函数与 Snowflake 对应函数直接兼容，为熟悉 Snowflake JSON 处理的用户提供无缝迁移体验。
...
[databendlabs/jsonb](https://github.com/databendlabs/jsonb) 库实现此二进制格式，以最小开销提供高性能 JSON 操作。
...
│  │ 4. 修改操作符：                                      │       │
│  │    data - 'user'  (删除键)                           │       │
│  │    data || '{"new_field":123}'  (连接操作)           │       │
│  └──────────────────────────────────────────────────────┘       │
│                                                                 │
└─────────────────────────────────────────────────────────────────┘
```

访问语法详情请参阅[变体文档](/sql/sql-reference/data-types/variant#accessing-elements-in-json)和[JSON 操作符文档](/sql/sql-commands/query-operators/json)。

### 丰富函数支持

Databend 提供全面的 JSON 数据处理函数，按功能分类：
