...
| [计算列](/sql/sql-commands/ddl/table/ddl-create-table#computed-columns)          | 查询           | 计算列通过从现有列派生新列来节省时间和精力：<br/>- 自动更新确保数据的准确性和一致性。<br/>- 高级分析和计算现在可以在数据库内执行。<br/>- 两种类型的计算列：存储和虚拟。虚拟列节省空间，因为它们在查询时即时计算。                                                                                                                                                                                                                                                      |
