...
TSV (Tab Separated Values) 是一种用于存储表格数据的简单文件格式，例如电子表格或数据库中的数据。TSV 文件格式与 CSV 非常相似，记录之间用换行符分隔，每个字段之间用制表符分隔。
...
- 有关更多 TSV 文件格式选项，请参阅 [TSV 文件格式选项](/sql/sql-reference/file-format-options#tsv-options)。
