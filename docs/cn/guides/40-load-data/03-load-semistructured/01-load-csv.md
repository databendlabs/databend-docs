...
CSV（逗号分隔值）是一种简单的文件格式，用于存储表格数据，如电子表格或数据库。CSV 文件是纯文本文件，包含以表格格式存储的数据，其中每一行在新行上表示，列由分隔符分隔。

以下示例显示了一个包含两个记录的 CSV 文件：
...
为了更精细的控制，例如在复制过程中转换数据，请使用 SELECT 语句。了解更多信息请参见[`SELECT from CSV`](../04-transform/01-querying-csv.md)。
