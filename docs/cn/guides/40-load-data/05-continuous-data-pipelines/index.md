...
数据管道可以自动地将来自不同来源的数据移动和更改到 Databend 中。它们确保数据流畅流动，对于快速和持续地处理和分析数据至关重要。

在持续数据管道中，一个名为 **变更数据捕获 (CDC)** 的特殊功能起着关键作用。通过 Databend，CDC 变得简单高效，只需通过 Streams 和 Tasks 执行几个简单的命令即可。

## 了解变更数据捕获 (CDC)

CDC 是一个流对象捕获应用于数据库表的插入、更新和删除的过程。它包括关于每个更改的元数据，从而可以基于修改后的数据执行操作。Databend 中的 CDC 跟踪源表中行级别的更改，创建一个“更改表”，反映两个事务时间点之间的修改。
