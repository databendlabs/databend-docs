...
Databend 的 `COPY INTO` 命令允许在加载过程中进行数据转换。这通过合并基本转换来简化您的 ETL 管道，从而无需使用临时表。

带有转换的 `COPY INTO` 的通用语法是：
...
有关更多详细信息，请参见 [COPY INTO](/sql/sql-commands/dml/dml-copy-into-table)。
...
-   **转换数据类型**：确保一致性和兼容性。
-   **执行算术运算**：生成新的派生数据。
-   **将数据加载到具有附加列的表**：将数据映射并插入到现有结构中。

## 教程

这些教程演示了加载期间的数据转换。每个示例都显示了从 Stage 文件加载。
...
将数据加载到列顺序不同的表中（例如，“age”在“name”之前）。
...
加载数据并转换列的数据类型（例如，“onboarded”为 `DATE`）。
