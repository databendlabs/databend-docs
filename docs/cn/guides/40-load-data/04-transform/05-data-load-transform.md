...
Databend 的 `COPY INTO` 命令允许在加载过程中进行数据转换。这通过整合基本转换来简化 ETL 管道，无需使用临时表。

`COPY INTO` 命令与转换结合使用的通用语法如下：
...
更多详情，请参阅 [COPY INTO](/sql/sql-commands/dml/dml-copy-into-table)。
