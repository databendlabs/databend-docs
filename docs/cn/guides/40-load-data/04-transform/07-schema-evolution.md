...
Schema Evolution 允许 Databend 在 `COPY INTO` 加载数据时，自动将源文件中存在但目标表中缺失的列添加到目标表。目前支持 **Parquet** 和 **JSON（NDJSON）**。
...
- **JSON（NDJSON）**：启用表选项后，`COPY INTO` 会使用 `AUTO` 采样值进行 schema 推断。可以选择设置 `SCHEMA_EVOLUTION = (...)` 来覆盖文件和记录采样限制。
...
Databend 的 JSON 行式加载使用 `TYPE = ndjson`。NDJSON 文件没有像 Parquet 那样的内嵌列式 schema，Databend 会先对文件内容进行采样，推断目标表中缺失的字段，再追加为可空列。
...
对 JSON（NDJSON）执行 Schema Evolution 时，Databend 会按以下规则推断新列：
...
- Schema Evolution 对 JSON 使用浅层推断：顶层字段值如果是对象或数组，会作为 `VARIANT` 列追加，不会递归展开为嵌套列。
