...
Databend 的 `COPY INTO` 命令允许在加载过程中进行数据转换。通过整合基本转换，可简化 ETL Pipeline（ETL 流水线），无需使用临时表。

有关语法，请参阅[查询与转换](./index.md)。
...
-   **转换数据类型**：确保一致性和兼容性。
-   **执行算术运算**：生成新的派生数据。
-   **将数据加载到具有额外列的表中**：将数据映射并插入到现有结构中。

## 教程

这些教程演示了在加载过程中进行数据转换。每个示例都展示了如何从一个暂存文件中加载数据。

### 准备工作

创建一个 Stage（暂存区）并生成一个 Parquet 示例文件：
