...
当在 SELECT 语句中使用 staged files 且没有可用的表名时，您可以为文件分配别名。这允许您将文件视为表，其字段用作表中的列。这在 SELECT 语句中使用多个表或选择特定列时非常有用。这是一个例子：
...
当使用 COPY INTO 从 staged file 复制数据时，Databend 会将 NDJSON 文件顶层的字段名称与目标表中的列名进行匹配，而不是依赖于列位置。在下面的示例中，表 _my_table_ 应具有与 NDJSON 文件中顶层字段名称相同的列定义：
...
假设您有以下具有相同 schema 的 Parquet 文件，以及一些其他格式的文件，这些文件存储在 Amazon S3 上名为 _databend-toronto_ 的 Bucket 中，区域为 _us-east-2_。
