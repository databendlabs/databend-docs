...
通过 AWS IAM Role，您可以在 Databend Cloud 中访问您自己的 AWS S3 buckets。这使您可以安全地访问您的数据并执行数据分析，而无需管理您的 AWS 凭证。
...
2. 前往 AWS 控制台：
   ...
   点击 `Create policy`，然后选择 `Custom trust policy`，并输入用于 S3 bucket 访问的策略文档：
...
:::info
恭喜！您现在可以使用 IAM Role 在 Databend Cloud 中访问您自己的 AWS S3 buckets。
:::
```