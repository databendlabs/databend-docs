...
Stage 还能提升文件管理效率。通过 [BendSQL](../../35-connect/00-sql-clients/bendsql.md)，一条命令即可完成文件上传或下载。加载数据时，可在 COPY INTO 命令中指定 Stage，实现文件读取和筛选；导出数据时，同样可以将文件写入 Stage。
...
<a id="filtering-staged-files-with-pattern"></a>
