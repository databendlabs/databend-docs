...
[Vector](https://vector.dev/) 是一个高性能的可观测性数据管道，使组织能够控制其可观测性数据。收集、转换所有日志、指标和追踪数据，并将它们路由到您今天想要的任何供应商以及您明天可能想要的任何其他供应商。Vector 可以在您需要的地方（而不是在您的供应商最方便的地方）大幅降低成本、实现新颖的数据丰富和数据安全。开源且比所有替代方案快 10 倍。

Vector 原生支持将数据传送到 [Databend 作为接收器](https://vector.dev/docs/reference/configuration/sinks/databend/)，这意味着 Vector 可以将数据发送到 Databend 以进行存储或进一步处理。Databend 充当 Vector 收集和处理的数据的目标。通过配置 Vector 使用 Databend 作为接收器，您可以无缝地将数据从 Vector 传输到 Databend，从而实现高效的数据分析、存储和检索。
