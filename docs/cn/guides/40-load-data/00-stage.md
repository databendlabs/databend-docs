...
Databend 使您能够轻松地从用户阶段或内部/外部阶段上传的文件中导入数据。为此，您可以首先使用 [BendSQL](../../30-sql-clients/00-bendsql/index.md) 将文件上传到一个阶段，然后使用 [COPY INTO](/sql/sql-commands/dml/dml-copy-into-table) 命令从阶段文件中加载数据。请注意，文件必须是 Databend 支持的格式，否则无法导入数据。有关 Databend 支持的文件格式的更多信息，请参见 [输入和输出文件格式](/sql/sql-reference/file-format-options)。

![image](/img/load/load-data-from-stage.jpeg)

以下教程提供了详细的逐步指导，帮助您有效地完成从阶段中的文件加载数据的过程。
...
按照本教程将样本文件上传到用户阶段，并从阶段文件中将数据加载到 Databend。
...
按照本教程将样本文件上传到内部阶段，并从阶段文件中将数据加载到 Databend。

### 步骤 1. 创建内部阶段

1. 使用 [CREATE STAGE](/sql/sql-commands/ddl/stage/ddl-create-stage) 命令创建一个内部阶段：
...
按照本教程将样本文件上传到外部阶段，并从阶段文件中将数据加载到 Databend。

### 步骤 1. 创建外部阶段

1. 使用 [CREATE STAGE](/sql/sql-commands/ddl/stage/ddl-create-stage) 命令创建一个外部阶段：
