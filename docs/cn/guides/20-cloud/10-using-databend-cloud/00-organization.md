...
本主题介绍了 Databend Cloud 中的组织及其成员的概念。
...
当您在注册过程中提供组织名称时，您将在 Databend Cloud 中创建一个组织，并将您的帐户作为管理员帐户。您还需要为新组织选择定价计划、云服务商和区域。有关更多信息，请参见 [入门](../00-new-account.md)。

![](@site/static/img/documents/getting-started/01.jpg)

:::tip
如果您是被已属于 Databend Cloud 中某个组织的用户邀请的，则文本框将显示该组织的名称。在这种情况下，您无法创建另一个组织。
:::
...
- 列出的角色显示了用户在被邀请时分配的角色。虽然可以在页面上更改这些角色，但无法使用 SQL 撤销它们。但是，您可以根据用户的电子邮件地址向用户授予其他角色或权限。这些用户帐户（由其电子邮件地址标识）也可以在 Databend Cloud 中充当 SQL 用户。例如：
...
- 如果您的组织采用的是试用版计划，则最多允许一个用户。在这种情况下，您将无法向其他成员发出邀请。
:::
```