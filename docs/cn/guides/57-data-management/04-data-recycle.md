...
在 Databend 中，当你运行 `DROP`、`TRUNCATE` 或 `DELETE` 命令时，数据并非真正被删除，而是允许通过时间回溯回到之前的状态。

数据分为两种类型：

- **历史数据**：时间回溯用于存储历史数据或已删除表中的数据。
- **临时数据**：系统用于存储溢出数据。
...
如果同时配置了本地和 S3 兼容的溢出存储，Databend 将遵循以下顺序：

1. 首先溢出到本地磁盘（如果已配置）。
2. 当本地磁盘空间不足时，溢出到远程 S3 兼容存储。
3. 如果既未配置本地存储也未配置外部 S3 兼容存储，则溢出到 Databend 的默认存储桶。
...
此示例设置 Databend 使用高达 1 TB 的本地磁盘空间进行溢出操作，同时保留 40% 的磁盘空间供系统使用：
...
参见 [VACUUM DROP TABLE](/sql/sql-commands/administration-cmds/vacuum-drop-table)。
...
参见 [VACUUM TABLE](/sql/sql-commands/administration-cmds/vacuum-table)。
...
参见 [VACUUM TEMPORARY FILES](/sql/sql-commands/administration-cmds/vacuum-temp-files)。