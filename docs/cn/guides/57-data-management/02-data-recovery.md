...
- **查看过去的数据使用情况：** 可以更轻松地查看数据在特定时间的使用或更改方式。
...
- **时间回溯的 SQL 扩展：** 在 SELECT 语句和 CREATE 命令中使用特殊的 SQL 子句，如 [`AT`](/sql/sql-commands/query-syntax/query-at)，以指定要访问的历史记录中的确切时间点。
...
Databend Cloud 中的 Fail-safe 是一项额外的安全功能，不同于时间回溯。它旨在保护您的数据，以防系统问题或安全事件。
