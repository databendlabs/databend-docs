...
| [Aggregating Index](/sql/sql-commands/ddl/aggregating-index)                      | 查询           | 使用聚合索引提高查询速度：<br/>- 通过预计算和索引的聚合来加速查询。<br/>- 自定义索引以满足您独特的数据分析需求。                                                                                                                                                                                                                                                                                                                                   |
...
| 存储加密                                                                         | 安全           | 增强服务器端数据加密的安全性，保护您的数据免受存储供应商的未经授权的访问：<br/>- 选择通过服务管理的密钥、KMS 管理的密钥或客户管理的密钥进行加密。选项可能因存储类型而异。<br/>- 目前在阿里云 OSS 上受支持。<br/>有关每个存储供应商的加密参数，请参阅 [部署指南](../../10-deploy/01-deploy/01-non-production/01-deploying-databend.md)。 |
...
本节比较了 Databend 社区版和 Databend 企业版在以下模块中的差异：
