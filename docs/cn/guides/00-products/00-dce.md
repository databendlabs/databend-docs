...
Databend 是一个用 Rust 构建的开源、弹性且具有工作负载感知能力的云数仓，为 Snowflake 提供了一种经济高效的替代方案。它专为全球最大数据集的复杂分析而设计。
...
- **Secondary Indexes**: 加快跨各种分析维度的数据定位和访问。

- **Complex Data Type Indexes**: 旨在加速复杂类型（例如半结构化数据）的数据处理和分析。

- **Segments**: Databend 有效地将数据组织成段，从而提高数据管理和检索效率。
