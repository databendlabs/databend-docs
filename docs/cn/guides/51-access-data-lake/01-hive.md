...
Databend 支持集成 [Apache Hive](https://hive.apache.org/) catalog，增强了其数据管理和分析的兼容性和多功能性。通过将 Apache Hive 强大的元数据和存储管理功能无缝集成到平台中，扩展了 Databend 的功能。
...
此示例演示了如何创建一个 catalog，该 catalog 配置为与 Hive Metastore 交互并访问存储在 Amazon S3 上，位于 's3://databend-toronto/' 的数据。
