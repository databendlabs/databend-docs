...
`GROUP BY CUBE` 是 [GROUP BY](index.md) 子句的扩展，类似于 [GROUP BY ROLLUP](group-by-rollup.md)。除了生成 `GROUP BY ROLLUP` 的所有行之外，`GROUP BY CUBE` 还添加了所有“交叉列表”行。小计行是进一步聚合的行，其值是通过计算用于生成分组行的相同聚合函数来导出的。
...
假设我们有一个 sales_data 表，其结构如下，并包含以下示例数据：
