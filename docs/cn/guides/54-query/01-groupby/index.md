...
GROUP BY 语句允许你基于相同的 group-by-item 表达式对行进行分组，然后将聚合函数应用于每个生成的分组。group-by-item 表达式可以包括列名或别名、对 SELECT 列表中位置的数字引用、通用表达式或 SELECT 列表中的所有非聚合项。

Databend 中的 GROUP BY 语句提供以下扩展，以实现更全面的数据分组和多功能的数据分析：
