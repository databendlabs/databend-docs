...
Databend 的 dictionary 功能提供了一种有效的方式来集成和查询来自 [支持的外部数据源](#supported-external-sources) 的数据，直接在 Databend 内部进行。通过充当内存中的键值存储，dictionary 能够快速访问外部数据，而无需复杂的数据管道或传统的 ETL 过程。

## Dictionary 的工作原理

在 Databend 中，您可以通过定义其结构并指定外部数据源来创建 dictionary。查询时，Databend 从外部数据源获取所需的数据。您可以使用 [DICT_GET](/sql/sql-functions/dictionary-functions/dict-get) 函数来高效地检索值，确保查询始终反映最新的数据。
