...
Databend 支持使用 WITH 子句的公用表表达式 (CTEs)，允许您定义一个或多个命名的临时结果集，供后续查询使用。“临时”一词意味着结果集不会永久存储在数据库 schema 中。它们充当仅对后续查询可访问的临时视图。

当执行带有 WITH 子句的查询时，将首先评估和执行 WITH 子句中的 CTE。这将生成一个或多个临时结果集。然后，使用 WITH 子句生成临时结果集来执行查询。

这是一个简单的演示，可帮助您了解 CTE 在查询中的工作方式：WITH 子句定义一个 CTE，并生成一个结果集，其中包含所有来自 Québec 省的客户。主查询从 Québec 省的客户中筛选出居住在 Montréal 地区的客户。
...
CTEs 简化了使用子查询的复杂查询，并使您的代码更易于阅读和维护。如果不使用 CTE，则前面的示例将如下所示：
...
在查询中使用 CTE 时，您可以使用 MATERIALIZED 关键字来控制 CTE 是内联还是物化。内联意味着 CTE 的定义直接嵌入在主查询中，而物化 CTE 意味着计算其结果一次并将其存储在内存中，从而减少重复的 CTE 执行。
...
在这种情况下，我们使用 MATERIALIZED 关键字，这意味着 CTE *customer_orders* 不会被内联。相反，CTE 的结果将在 CTE 定义的执行期间计算并存储在内存中。在执行主查询中的 CTE 的两个实例时，Databend 将直接从内存中检索结果，从而避免冗余计算并可能提高性能。
...
2. **递归查询执行**：此查询使用前一次迭代的结果集（从 R0 开始）作为输入，并生成一个新的结果集 (Ri+1)。

3. **迭代和组合**：递归执行迭代地继续。来自递归查询的每个新结果集 (Ri) 成为下一次迭代的输入。此过程重复进行，直到递归查询返回一个空的结果集，表明已满足终止条件。

4. **最终结果集形成**：使用 `UNION ALL` 运算符，来自每次迭代的结果集（R0 到 Rn）被组合成一个单一的结果集。`UNION ALL` 运算符确保每个结果集中的所有行都包含在最终组合的结果中。

5. **最终选择**：最后的 `SELECT ...` 语句从 CTE 中检索组合的结果集。此语句可以对组合的结果集应用额外的过滤、排序或其他操作，以生成最终输出。

## 使用示例

### 非递归 CTE

假设您管理着位于大多伦多地区不同区域的几家书店，并使用一个表来保存它们的商店 ID、区域和上个月的交易量。
...
以下代码将销售额低于各自区域平均销售额的商店的销售额更新为 0：
