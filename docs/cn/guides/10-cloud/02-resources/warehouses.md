...
使用计算集群会产生费用。更多详情请参阅 [计算集群定价](/guides/cloud/overview/pricing#warehouse-pricing)。
...
查看 Small、Medium、Large 三种规格在同一工作负载下的实际性能对比，请访问 [TPC-H SF1000（1TB）Warehouse 规格基准测试](/guides/benchmark/tpch-sf1000) 页面。

**数据规模：** TPC-H Scale Factor 1000（SF1000）表示约 1TB 的生成数据，包含 8 张 TPC-H 标准表，总行数约 60 亿行。

:::note[总览]

| Warehouse 规格 |    总耗时 | 相对 Small 加速比 |
| -------------- | --------: | ----------------: |
...
:::tip
您也可以通过 SQL 命令管理计算集群，详见 [Warehouse DDL 命令](/sql/sql-commands/ddl/warehouse)。
:::
...
您可以为计算集群添加标签（Tags），以便按环境、团队或成本中心等维度进行分类管理。标签以键值对形式存储，在计算集群列表中可见，并支持按键或值进行筛选和排序。
...
在创建或编辑计算集群时，展开 **标签** 区域并输入键值对即可添加标签：

![alt text](@site/static/img/cloud/tags.png)

标签在计算集群列表中以 `key: value` 格式显示，可用于按键或值筛选计算集群。
...
:::note
计算集群访问控制默认**未启用**。如需开启，请前往 **工单** > **创建新工单** 提交申请。
:::

在创建或修改计算集群时，您可以在 **高级选项** 中选择需要分配的角色：
...
- 您可以使用 [GRANT](/sql/sql-commands/ddl/user/grant) 命令将角色授予用户（Databend Cloud 登录邮箱或 SQL 用户），或者在邀请用户加入组织时直接分配角色。详情请参阅 [邀请新成员](organization.md#inviting-new-members)。以下示例将 `manager` 角色授予邮箱 `name@example.com`，使其能够访问所有分配给 `manager` 角色的计算集群：
...
:::note
多集群计算集群默认**未启用**。如需开启，请前往 **工单** > **创建新工单** 提交申请。该功能仅对 Databend Cloud **商业版 (Business)** 和 **专用版 (Dedicated)** 用户开放。
:::
...
MySQL Endpoint 功能允许计算集群接受来自仅支持 MySQL 协议的 BI 工具和应用程序的连接，例如 Tableau、Grafana 或其他 MySQL 兼容客户端。

:::note
MySQL Endpoint 默认**未启用**。如需开启，请前往 **工单** > **创建新工单** 提交申请。
:::

### 启用 MySQL Endpoint

您可以在创建或修改计算集群时启用 MySQL Endpoint，该选项位于 **Advanced Options** 中，以开关形式呈现。

![alt text](@site/static/img/cloud/mysql-endpoint.png)

:::caution
启用 MySQL Endpoint 后，该计算集群的**自动暂停（Auto Suspend）将被强制禁用**（设为 0）。这意味着计算集群将持续运行并持续产生费用，即使处于空闲状态。请根据实际使用情况合理规划。
:::

### 通过 MySQL 协议连接

启用后，您可以使用任何 MySQL 兼容客户端，通过 **Connect** 对话框中显示的 MySQL 连接信息连接到计算集群。这对于不原生支持 Databend 协议的工具集成非常有用。
...
1. 在 **概览**（概览）页面点击 **连接**（连接）。
2. 选择您要连接的 Database（数据库）和 Warehouse（计算集群）。连接信息将根据您的选择自动更新。
3. 连接详情包含一个名为 `cloudapp` 的系统生成 SQL 用户及其随机密码。Databend Cloud 不会存储此密码，请务必复制并妥善保存。如遗忘，可点击 **重置** 生成新密码 (需要 Admin 才能重置)。

![alt text](@site/static/img/documents_cn/warehouses/databend_cloud_dsn.gif)

### 连接字符串格式

点击 **连接** 时，Databend Cloud 会自动生成连接字符串：
...
默认情况下，所有连接到 Databend Cloud 计算集群的流量均使用 TLS 加密。对于安全性要求更高的企业用户，可以使用 [AWS PrivateLink](/guides/cloud/security/private-link) 在您的 VPC 和 Databend Cloud 之间建立私有连接。
