...
Databend Cloud 对工作区输出区域显示的查询结果应用以下默认限制：

| 配置项 | 默认值 | 说明 |
|---|---|---|
| 最大显示行数 | 10,000 行 | 预览区最多显示前 10,000 行数据。 |
| 最大显示列数 | 200 列 | 预览区最多显示前 200 列数据。 |
| 单元格最大显示长度 | 3,000 字符 | 超出部分将在显示时被截断。 |

行数和列数限制为固定值，不可调整。如需修改单元格最大显示长度，可点击结果区右下角的设置图标，选择合适的值（3K 至无限制）。请注意，设置过大的值或选择**无限制**时，若结果集数据量较大，可能导致浏览器卡顿甚至崩溃。
