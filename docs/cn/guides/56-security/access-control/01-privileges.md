...
权限是一种允许执行特定操作的许可。在Databend中，用户必须拥有特定的权限才能执行特定的操作。例如，查询表时，用户需要对该表拥有`SELECT`权限。同样，要读取阶段中的数据集，用户必须拥有`READ`权限。

在Databend中，用户可以通过两种方式获得权限。一种方法是直接将权限授予用户。另一种方法是将权限首先授予角色，然后将该角色分配给用户。
...
Databend提供了一系列权限，允许您对数据库对象进行精细控制。Databend的权限可以分为以下几类：
...
| CREATEUSER | 创建用户。                                                                                                     |
| DROPROLE   | 删除角色。                                                                                                     |
| SUPER      | 终止查询。设置或取消设置。操作阶段、目录或共享。调用函数。COPY INTO 阶段。                                     |
...
| 权限       | 描述                                                                                                   |
|:----------|:--------------------------------------------------------------------------------------------------------------|
| WRITE     | 写入阶段。例如，COPY INTO 阶段，预签名上传或删除阶段。                                                       |
| READ      | 读取阶段。例如，列出阶段，查询阶段，从阶段 COPY INTO 表，预签名下载。                                       |
| ALL       | 授予指定对象类型的 READ, WRITE 权限。                                                                         |
| OWNERSHIP | 授予对阶段的完全控制。同一时间只有一个角色可以对特定对象持有此权限。                                       |
...
| 权限       | 描述                                                                                                 |
|:----------|:------------------------------------------------------------------------------------------------------------|
| USAGE     | 可以使用 UDF。例如，COPY INTO 阶段，预签名上传。                                                           |
| ALL       | 授予指定对象类型的 READ, WRITE 权限。                                                                         |
