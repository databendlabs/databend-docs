...
权限是执行操作的许可。用户必须具有特定的权限才能在 Databend 中执行特定的操作。例如，当查询表时，用户需要对该表具有 `SELECT` 权限。类似地，要读取 Stage 中的数据集，用户必须具有 `READ` 权限。
...
在以下示例中，撤销角色 'writer' 在 'default' schema 中所有对象的权限。在此之后，显示用户 'eric' 的已授予权限。
...
请注意，一旦您拥有对数据库的以下任何权限或对数据库中表的任何权限，您就可以使用 [USE DATABASE](/sql/sql-commands/ddl/database/ddl-use-database) 命令来指定数据库。
