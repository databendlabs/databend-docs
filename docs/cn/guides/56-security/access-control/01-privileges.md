...
要授予权限，您有两种选择：您可以直接将权限授予用户，也可以先将权限授予角色，然后将该角色授予用户。在以下示例中，权限直接授予给用户 'david'。'david' 被创建为一个新用户，密码为 'abc123'，然后 'default' schema 中所有对象的权限都直接授予给 'david'。最后，显示了授予给 'david' 的权限。
...
| CREATEUSER | 创建用户。                                                                                                           |
| DROPROLE   | 删除角色。                                                                                                           |
| SUPER      | 终止查询。设置或取消设置。操作 Stage、Catalog 或 Share。调用函数。COPY INTO 一个 Stage。                                                                   |
...
| INSERT    | 将行插入到表中。COPY INTO 一个表。                                                                                                 |
| SELECT    | 从表中选择行。SHOW CREATE 一个表。DESCRIBE 一个表。                                                                                     |
| UPDATE    | 更新表中的行。                                                                                                         |
| SUPER     | 优化或分析表。                                                                                                         |
...
请注意，一旦您拥有以下任何数据库权限或数据库中任何表的权限，您可以使用 [USE DATABASE](/sql/sql-commands/ddl/database/ddl-use-database) 命令来指定数据库。
...
| 权限        | 描述                                                                                                           |
|:----------|:--------------------------------------------------------------------------------------------------------------|
| WRITE     | 写入到 Stage。例如，copy into 一个 Stage，预签名上传或删除一个 Stage                                                                 |
| READ      | 读取一个 Stage。例如，list stage，query stage，copy into table from stage，预签名下载                                                              |
...
| 权限        | 描述                                                                                                             |
|:----------|:--------------------------------------------------------------------------------------------------------------|
| USAGE     | 可以使用 UDF。例如，copy into 一个 Stage，预签名上传                                                                      |
| ALL       | 授予指定对象类型的 READ、WRITE 权限。                                                                                             |
| OWNERSHIP | 授予对 UDF 的完全控制权。一次只能有一个角色拥有对特定对象的此权限。                                                                                 |

> 注意：
> 
> 1. 如果 UDF 已经被持续折叠，则不检查 UDF 认证。
> 2. 如果 UDF 是插入中的一个值，则不检查 UDF 认证。
