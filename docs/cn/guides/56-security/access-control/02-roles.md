...
:::note
目前，对于用户定义函数 (UDF) 和 Stage，Databend 默认不强制执行基于角色的访问控制 (RBAC) 检查。但是，如果您需要对这些对象执行 RBAC，您可以选择手动设置 `SET GLOBAL enable_experimental_rbac_check=1` 来全局启用它。
...
| 内置角色      | 描述                                                                                                                            |
|---------------|----------------------------------------------------------------------------------------------------------------------------------------|
| account-admin | 拥有所有权限，充当所有其他角色的父角色，并支持无缝切换到租户中的任何角色。                                                                                              |
| public        | 不继承任何权限，将所有角色视为其父角色，并允许任何角色切换到 public 角色。                                                                                  |

要在 Databend Cloud 中将 `account-admin` 角色分配给用户，请在邀请用户时选择该角色。您也可以在用户加入后将该角色分配给用户。如果您使用的是 Databend Community Edition 或 Enterprise Edition，请首先在部署期间配置 `account-admin` 用户，然后根据需要将该角色分配给其他用户。有关配置管理员用户的更多信息，请参见 [配置管理员用户](../../10-deploy/04-references/01-admin-users.md)。
...
可以向用户授予 Databend 中的多个角色。这些角色分为活动角色和辅助角色：

- 活动角色是用户当前会话的活动主角色，可以使用 [SET ROLE](/sql/sql-commands/ddl/user/user-set-role) 命令进行设置。

- 辅助角色是提供额外权限的其他角色，默认情况下处于活动状态。用户可以使用 [SET SECONDARY ROLES](/sql/sql-commands/ddl/user/user-set-2nd-roles) 命令激活或停用辅助角色，以临时调整其权限范围。

## 账单角色

除了标准的内置角色之外，您还可以在 Databend Cloud 中创建一个名为 `billing` 的自定义角色，专门满足财务人员的需求。角色 `billing` 仅提供对与账单相关的信息的访问权限，从而确保财务人员可以查看必要的财务数据，而不会接触到其他与业务相关的页面。
...
CREATE ROLE billing;
```
角色名称不区分大小写，因此 `billing` 和 `Billing` 被认为是相同的。有关设置和分配角色 `billing` 的详细步骤，请参见 [授予财务人员访问权限](/guides/cloud/manage/costs#granting-access-to-finance-personnel)。
