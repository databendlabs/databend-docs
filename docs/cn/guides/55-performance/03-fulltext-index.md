...
全文索引（倒排索引）通过建立词汇到文档的映射关系，无需全表扫描即可实现海量文档集合的毫秒级文本检索。
...
| 问题 | 影响 | 全文索引解决方案 |
|------|------|----------------|
| **LIKE 查询缓慢** | `WHERE content LIKE '%keyword%'` 需要扫描全表 | 直接定位词汇，跳过无关文档 |
| **全表扫描** | 每次文本搜索都读取所有行 | 仅读取包含搜索词的文档 |
| **搜索体验差** | 用户需等待数秒/分钟获取结果 | 亚秒级搜索响应 |
| **搜索功能有限** | 仅支持基础模式匹配 | 支持高级功能：模糊搜索、相关性评分 |
| **资源消耗高** | 文本搜索占用大量 CPU/内存 | 索引搜索仅需最小资源 |

**示例**：在 1000 万条日志中搜索 "kubernetes error"。无全文索引需扫描全部 1000 万行，有全文索引可直接定位约 1000 条匹配文档。
...
当搜索 "kubernetes error" 时，索引直接定位同时包含两个词汇的文档（101, 1847），无需扫描全表。
...
### 模糊搜索
```sql
-- 即使存在拼写错误也能查找文档（fuzziness=1 允许 1 个字符差异）
...
| 命令 | 用途 | 使用场景 |
|------|------|----------|
| `CREATE INVERTED INDEX name ON table(column)` | 创建新全文索引 | 初始设置 - 新数据自动索引 |
| `REFRESH INVERTED INDEX name ON table` | 索引已有数据 | 仅需对存量数据执行一次 |
...
*全文索引是需要在海量文档集合中实现快速、复杂文本搜索应用的必备功能。*