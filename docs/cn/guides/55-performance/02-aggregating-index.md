...
Databend 中的自动刷新机制可能会影响重要数据加载的持续时间。这是因为 Databend 会保留数据加载结果，直到自动刷新的聚合索引已更新以反映最新结果。建议 Databend Cloud 用户使用手动刷新机制。这是因为 Databend Cloud 会在后台自动更新聚合索引，即使对于那些在没有 SYNC 关键字的情况下创建的聚合索引，以响应表数据中的更改。
:::
...
此示例演示了聚合索引的利用，并说明了它们对查询执行计划的影响。
