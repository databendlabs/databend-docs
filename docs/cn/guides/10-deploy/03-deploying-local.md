...
3. 在 **scripts** 文件夹中运行脚本 **start.sh**：
    ...
    MacOS 可能会提示错误，称“*databend-meta 无法打开，因为 Apple 无法检查其是否含有恶意软件。*”要继续，请在 Mac 上打开 **系统设置**，在左侧菜单中选择 **隐私与安全**，然后在右侧的 **安全** 部分为 databend-meta 点击 **仍要打开**。对于 databend-query 的错误也要做同样的操作。
...
```shell
sudo chown -R $(whoami) databend-meta
sudo chown -R $(whoami) databend-query
```
:::
```
