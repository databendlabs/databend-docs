...
- 您可以在 GitHub 上找到为各种部署环境设置 Databend 的[示例配置文件](https://github.com/datafuselabs/databend/tree/main/scripts/ci/deploy/config)。这些文件仅用于内部测试。请勿为您自己的目的修改它们。但是，如果您有类似的部署，当编辑您自己的配置文件时参考它们是个好主意。
...
| auth_string  | 认证字符串（例如，密码的 SHA-1 或 SHA-256 散列值）。 |
...
| file_limit       | 指定日志保留期限（单位：小时），file_limit 默认为 48 小时。  |
...
| password                      | 用于连接 Meta 服务的密码。Databend 建议使用环境变量 META_PASSWORD 来提供密码。默认："root"。 |
| endpoints                     | 设置一个或多个 meta 服务器端点，此查询服务器可以连接到。为了 Meta 的稳定连接，如果可能，包括集群中的多个 meta 服务器作为备份。示例：["192.168.0.1:9191", "192.168.0.2:9191"]。默认：["0.0.0.0:9191"]。 |
| client_timeout_in_second      | 设置在终止尝试连接到 meta 服务器之前的等待时间（秒）。默认：60。                         |
| auto_sync_interval            | 设置此查询服务器应该多久自动从集群中的 meta 服务器同步端点（秒）。启用后，Databend-query 定期联系 Databend-meta 服务器以获取 grpc_api_advertise_host:grpc-api-port 列表。要禁用同步，将其设置为 0。默认：60。 |
| unhealth_endpoint_evict_time  | 不查询不健康的 meta 节点端点的内部时间（秒）。默认：120。                               |
...
| allow_insecure   | 默认为 false，如果您使用 minio 或从 http:// 前缀 url 复制，请将其设置为 true。            |

> 使用 copy into 从 stage 或 http:// 前缀 url 加载数据报告错误：不允许从不安全的存储复制，请设置 `allow_insecure=true`
...
| presign_endpoint_url | 阿里巴巴云OSS预签名操作的URL端点。                              |
| root                 | 阿里巴巴云OSS的根目录。                                         |
