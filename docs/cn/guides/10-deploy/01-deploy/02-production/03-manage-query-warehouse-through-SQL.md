...
description: 如何通过 SQL 在本地管理可伸缩和灵活的查询计算集群。
---
...
本主题介绍如何通过 SQL 在本地管理可伸缩和灵活的查询计算集群。

## 了解计算集群

![本地部署计算集群](/img/deploy/on-premise-warehouse.png)
...
- **计算集群**: 用户用来执行 SQL 查询的计算资源集合。它可以在内部包含多个集群，以处理并发查询工作负载。

- **集群**: 代表一组计算资源的抽象概念。每个 SQL 查询都将尝试使用所有节点的资源（包括 CPU、内存、磁盘等）来完成查询。一个计算集群由多个集群组成，每个集群在物理上是隔离的。当计算集群需要增加并发处理能力时，可以添加新的集群。在之前静态配置的 `tenant_id` 和 `cluster_id` 部署中，计算集群是一个名为 `cluster_id` 的单集群计算集群。
...
#### 向已有的计算集群添加 cluster，cluster 节点来自不同的节点组
```sql
