...
请注意，上述命令还设置了根用户凭据（ROOTUSER/CHANGEME123），您将需要在下一步中提供这些凭据以进行身份验证。如果您在此时更改了根用户凭据，请确保在整个过程中保持一致性。
...
要与 Databend 建立连接，您将在此步骤中使用 BendSQL CLI 工具。有关如何安装和操作 BendSQL 的说明，请参见 [BendSQL](../../../30-sql-clients/00-bendsql/index.md)。

1. 要使用 SQL 用户 (databend/databend) 与 Databend 建立连接，请运行以下命令：
...
1. 配置管理员用户。您将使用此账户连接到 Databend。有关更多信息，请参见 [配置管理员用户](../../04-references/01-admin-users.md)。以此示例，取消注释以下行以选择此账户：
...
```
:::tip
如果在尝试启动 Databend 时遇到后续的错误消息：

```shell
==> query.log <==
: No getcpu support: percpu_arena:percpu
: option background_thread currently supports pthread only
Databend Query 启动失败，原因：Code: 1104, Text = 无法创建 appender: Os { code: 13, kind: PermissionDenied, message: "Permission denied" }。
```
