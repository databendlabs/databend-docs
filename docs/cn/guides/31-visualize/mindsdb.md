...
数据库中的数据是一项宝贵的资产。[MindsDB](https://mindsdb.com/) 使您能够使用数据并进行预测。它通过将机器学习引入数据库来加速 ML 开发过程。借助 MindsDB，您可以构建、训练、优化和部署 ML 模型，而无需其他平台。

Databend 和 Databend Cloud 都可以与 MindsDB 集成作为数据源，从而将机器学习功能引入 Databend。以下教程展示了如何与 MindsDB 集成并进行数据预测，以 [Air Pollution in Seoul](https://www.kaggle.com/datasets/bappekim/air-pollution-in-seoul) 数据集为例。
...
运行以下 SQL 语句以在数据库 `default` 中创建表，并使用 COPY INTO 命令加载 [Air Pollution in Seoul](https://www.kaggle.com/datasets/bappekim/air-pollution-in-seoul) 数据集：
...
在 Databend Cloud 中打开一个工作表，然后运行以下 SQL 语句以在数据库 `default` 中创建表，并使用 COPY INTO 命令加载 [Air Pollution in Seoul](https://www.kaggle.com/datasets/bappekim/air-pollution-in-seoul) 数据集：
