...
`databend-tableau-connector-jdbc` 连接器通过其 JDBC 驱动程序提供更快的性能，尤其是在创建 Extracts 时，并且作为一个跨平台 jar 文件更容易安装，从而消除了特定于平台的编译。它允许您微调 SQL 查询以实现标准的 Tableau 功能，包括多个 JOIN 和使用 Sets，并提供用户友好的连接对话框，以实现无缝集成体验。

## 教程 1：与 Databend 集成（通过 Other Databases (JDBC) 接口）

在本教程中，您将部署本地 Databend 并将其与 [Tableau Desktop](https://www.tableau.com/products/desktop) 集成。在开始之前，请 [下载](https://www.tableau.com/products/desktop/download) Tableau Desktop，然后按照屏幕上的说明完成安装。
...
1. 启动 Tableau Desktop，然后在侧边栏中选择 **Other Database (JDBC)**。这将打开一个窗口，如下所示：
...
1. 从连接器的 [Releases](https://github.com/databendcloud/databend-tableau-connector-jdbc/releases) 页面下载最新的 **databend_jdbc.taco** 文件，并将其保存到 Tableau 的连接器文件夹中：
...
在本教程中，您将 Databend Cloud 与 [Tableau Desktop](https://www.tableau.com/products/desktop) 集成。在开始之前，请 [下载](https://www.tableau.com/products/desktop/download) Tableau Desktop，然后按照屏幕上的说明完成安装。
