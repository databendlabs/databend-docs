...
在 databend 内部使用的配置实例。所有逻辑 **SHOULD** 都应该针对 `inner` 配置实现。

**`outer`**

配置实例用作 databend 的前台。它们将转换为 `inner` 配置。其他模块 **SHOULD NOT** 依赖于 `outer` 配置。
...
所有维护通知 **SHOULD** 都应用于 `outer` 配置结构。

- 添加配置：使用新的默认值添加是兼容的，否则是被禁止的。
...
如参考中所述，`protobuf` 使用的配置是另一个 `outer` 配置。它应该自己处理版本。基于 databend `common-proto-conv` 的当前状态，我们将保留所有字段，直到我们决定增加 `OLDEST_COMPATIBLE_VER`。
...
我们可以引入版本化的配置，以允许用户指定配置版本：

- 配置文件：`version=42`
- 配置环境变量：`export CONFIG_VERSION=42`
- 参数：`--config-version=42`

假设兼容的更改发生，因为添加了一个新的配置条目。databend 将确保该条目具有默认值。

假设发生了不兼容的更改，例如配置被删除/重命名/更改。databend 增加了配置版本。旧版本仍将通过指定的版本加载，并在内部转换为最新配置。还将为已删除的配置字段打印 `DEPRECATED` 警告。因此，用户可以决定是否迁移它们。
...
为了获得最佳情况，我们可以通过版本 42 从环境变量加载，然后通过版本 23 从配置文件加载。
```