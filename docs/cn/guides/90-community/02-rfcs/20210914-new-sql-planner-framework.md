...
`DataSchema` 中的每个 `DataField` 都用唯一的 `name` 字符串标识。目前，`name` 仅表示列名，因此很难使用此抽象来实现 `JOIN`。我们将在后面的章节中讨论此问题的详细解决方案。
...
有趣的是，规则系统（转换和实现）与探索引擎和成本模型解耦，这意味着可以很容易地构建一个没有 CBO（基于成本的优化）的启发式优化器。一旦我们要实现 CBO，规则系统就可以重用。
...
- 引入一个新框架来支持规划 `JOIN` 和相关子查询
- 引入一个规则系统，使开发人员可以轻松编写转换规则

# 设计细节

## 架构

在当前的实现中，SQL 查询将按如下方式处理：
...
4. `Interpreter` 将 `PlanNode` 作为输入，并将其解释为由 `Processor` 组成的、可执行的 `Pipeline`
5. Executor 将使用特定的运行时执行 `Pipeline`

在我们的新框架中，`PlanParser` 将被重构为两个组件：

- `Parser`: 将 SQL 文本解析为统一的 AST 表示形式，该表示形式已在 [此 PR](https://github.com/databendlabs/databend/pull/1478) 中引入
- `Binder`: 将 AST 中出现的变量与数据库中的对象（例如，表、列等）绑定，并执行语义检查（名称解析、类型检查）。将生成计划树的逻辑表示形式

此外，将引入一个新的优化器，其中包含一个 `rule system` 以替换当前的优化器。

## Binder

由于 SQL 查询中可能存在许多语法上下文，因此我们需要一种方法来跟踪它们之间的依赖关系以及 `name` 在不同上下文中的可见性。
...
因此，在名称解析之后，每个变量都将与唯一的 `ColumnIndex` 绑定，而不是与字符串绑定，因此我们无需担心诸如重复名称之类的问题。

在绑定过程中，我们需要跟踪绑定的状态。该状态可能包含以下信息：

- 当前上下文中可见的列，在处理通配符结果集（`SELECT * FROM t`）时使用
- 上下文中的列是否为 group by 键
- 列是否为派生列（即，像 `a+1 AS a` 这样的投影）
- 变量是否来自当前子查询，以标识相关的列引用

为了维护状态，我们提出了一个数据结构 `BindContext`（此名称受到 CMU Peloton 的 `BinderContext` 的启发，在我看来这是一个非常合适的名称）。
...
让我们看一下 `BindContext` 如何处理相关子查询。

相关子查询表明子查询依赖于来自外部上下文的列。

有一个规范的 `Apply` 运算符来执行相关子查询，它将为每个元组评估子查询表达式（如交叉 join）。虽然大多数相关子查询可以被去相关为 join（例如，半连接、反半连接等）。
...
如果在父上下文中找到该列，那么我们可以确认此子查询是一个相关子查询，并且与父上下文绑定的列引用是相关列。
...
SQL 优化基于关系代数的等价性。有许多不同的定理和引理可以帮助我们确定两个关系代数树是否在逻辑上等价。通过一组等价的关系表达式，我们可以使用成本模型对它们进行评估，并找到最佳表达式。
...
在 Cascades 查询优化器中，SQL 查询将被转换为一个树状结构 `Expression`，其中关系运算符（`Operator`）作为其节点。
...
每个转换规则都描述了它可以应用于的关系运算符，我们称之为 `rule pattern`。优化器将提供一种方案来遍历 `Expression`，检查是否有任何规则可以应用于子 `Expression`，然后对匹配的规则应用转换。

`Expression` 的交替形式是在转换过程中生成的。例如：

```sql
SELECT * FROM t INNER JOIN t1 ON t.a = t1.a;
```

使用 `JoinCommutativity` 规则，上面的 SQL 可以转换为等效的 SQL：

```sql
SELECT * FROM t1 INNER JOIN t ON t.a = t1.a;
```

为了减少由转换生成的重复 `Expression`，Cascades 使用自上而下的方法来枚举交替形式。

引入了一个结构 `Memo` 来存储交替形式。每个 `Memo` 由 `Group` 组成，每个 `Group` 是一组等效的 `Expression`。

与我们上面提到的 `Expression` 不同，`Group` 中的 `Expression` 将 `Group` 作为其子节点，而不是 `Expression`，以便等效的 `Expression` 可以共享子节点候选项。
...
现在您已经掌握了 Cascades 优化器框架的基本知识。虽然 Cascades 还有许多其他重要的概念，但这个简短的介绍足以让您理解本 RFC。
...
`HeuristicOptimizer`，优化器将转换应用于 `SExpr` 并且不生成交替形式。
...
4. `CascadesOptimizer` 接受 `HeuristicOptimizer` 的输出，构建 `Memo`，应用规则，并返回最佳 `SExpr`
5. `Executor` 接受 `CascadesOptimizer` 生成的物理 `SExpr`，并构建可执行的 `Processor`
