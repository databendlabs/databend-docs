...
然而，对于需要强大灾难恢复计划的企业用户来说，这还不够。这些用户要么对跨大陆灾难恢复有重大需求，要么必须遵守严格的监管要求。
...
本 RFC 介绍了通过提供强大的备份和恢复解决方案，使 Databend 能够从灾难（例如元数据或数据丢失）中恢复的第一步。我们提出的产品，暂定名为 `bendsave`，将允许用户高效地备份和恢复元数据和数据。

*该产品的名称尚未确定，我们称之为 `bendsave`*
...
块 ID 由块内容的 SHA-256 校验和生成。因此，如果之前已备份，我们可以重用相同的块。
