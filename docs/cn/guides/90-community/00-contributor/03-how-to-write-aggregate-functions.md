...
- **accumulate** 用于 `SingleStateAggregator`。也就是说，整个块可以在没有任何键的情况下在单个状态下聚合。例如，当 `selecting count(*) from t` 时，查询中没有分组列，将调用 `accumulate` 函数。
...
作为一个优秀的开发人员，您总是会测试您的代码，不是吗？完成新的聚合函数后，请添加单元测试和逻辑测试。
