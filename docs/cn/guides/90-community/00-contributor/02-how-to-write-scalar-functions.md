...
标量函数为每一行返回一个单一的值，而不是一个结果集。标量函数可以用于查询或 SET 语句中的大多数位置（除了 FROM 子句）。
...
`funcs` 和 `factories` 都存储已注册的函数。`funcs` 接受固定数量的参数（目前从 0 到 5），`register_0_arg`、`register_1_arg` 等等。`factories` 接受可变长度的参数（例如 concat），并调用函数 `register_function_factory`。
...
签名包括函数名称、参数类型、返回类型和函数属性（目前不可用，保留用于函数）。特别要注意的是，注册时函数名称需要小写。一些 token 通过 `src/query/ast/src/parser/token.rs` 进行转换。
