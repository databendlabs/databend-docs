...
For more information about the command, see [`COPY INTO <location>`](/doc/sql-reference/file-format-options). To view the list of supported file formats that can be used to save the exported data, see [Input & Output File Formats](/doc/sql-reference/file-format-options).
...
Create an external stage named `unload` with the [CREATE STAGE](/doc/sql-commands/ddl/stage/ddl-create-stage) command:
...
Show the exported file with the [LIST STAGE](/doc/sql-commands/ddl/stage/ddl-list-stage) command:
