...
- Consume S3 `ObjectCreated` events and write the corresponding object data into Databend
...
| **Role ARN** | Yes | IAM Role ARN in your AWS account that Databend is allowed to assume |
| **External ID** | Yes | Organization ID from the Databend Cloud console, used in the IAM Role trust policy |
...
:::note
SQS (S3) ingestion uses the AssumeRole model. You do not need to provide AWS Access Key or Secret Key to Databend. Instead, create an IAM Role in your AWS account and allow Databend platform roles to obtain temporary credentials through `sts:AssumeRole` in the role trust policy.
:::
...
4. Create an IAM Role that allows Databend platform roles to access it through `sts:AssumeRole`.
...
Prepare the following variables first. `AWS_REGION` must be the Region where both the S3 bucket and SQS queue are located. `EXTERNAL_ID` is the organization ID from the Databend Cloud console.
...
Confirm that `QueueArn` points to the target SQS queue, `Events` includes `s3:ObjectCreated:*`, and `FilterRules` matches the `Object Key Prefix` / `Object Key Suffix` configured in the Databend data source.

## Step 4: Create an IAM Role for Databend to Assume

Generate `trust-policy.json`. `ExternalId` is the organization ID from the Databend Cloud console.
...
:::note
`receive-message` does not delete the message automatically. It only hides the message temporarily during the visibility timeout. If you want Databend to consume this test message later, do not delete it manually. Wait for the visibility timeout to expire before testing data source connectivity.
:::

## Information to Provide to Databend

After completing the AWS-side configuration, fill in the following information when creating the data source in Databend Cloud:

| Parameter | Description |
|-----------|-------------|
| `role_arn` | IAM Role ARN in your AWS account that Databend is allowed to assume |
| `external_id` | Organization ID from the Databend Cloud console |
...
- The bucket, prefix, and suffix in the S3 notification should match the Databend data source configuration.
