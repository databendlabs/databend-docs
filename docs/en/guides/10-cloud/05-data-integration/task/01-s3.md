...
This page describes how to create an Amazon S3 integration task that imports files from an S3 bucket into Databend. CSV, Parquet, and NDJSON file formats are supported, and the task can be configured for one-time import or continuous ingestion.

If you need to create reusable AWS credentials first, see [Amazon S3 - Credentials](../datasource/01-aws.md).
