...
Databend defaults to `SET SECONDARY ROLES ALL` — all granted roles are active in the session immediately after login. The policy CASE is evaluated top-down, so the first matching branch wins.
...
- Databend defaults to `SECONDARY ROLES ALL` after login — all granted roles are active in the session.
...
- Find policy usage with [`POLICY_REFERENCES`](/sql/sql-functions/table-functions/policy-references): `POLICY_REFERENCES(POLICY_NAME => '<policy_name>')`.
