...
To restrict which documents a role can retrieve during vector search, attach a [Row Access Policy](/guides/security/data-protection/row-access-policy#vector--rag-document-visibility) so visibility is enforced by the engine, not by stuffing document IDs into the query.
