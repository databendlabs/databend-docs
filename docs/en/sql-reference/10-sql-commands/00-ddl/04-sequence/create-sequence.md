...
`enable_experimental_sequence_privilege_check` governs sequence-level access control. It is disabled by default. When disabled, sequence creation solely requires the user to possess superuser privileges, bypassing detailed RBAC checks. When enabled, granular permission verification is enforced during sequence establishment.
