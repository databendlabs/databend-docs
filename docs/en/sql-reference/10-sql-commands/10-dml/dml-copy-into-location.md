...
`LANCE` is supported only in `COPY INTO <location>`. Databend writes a Lance dataset directory under the target path, not a standalone file.
...
- With `TYPE = LANCE` and `USE_RAW_PATH = FALSE`, Databend appends the query ID to the target path and creates a separate dataset root for each export.
:::
...
For a complete end-to-end example, including validation with Python `lance`, see [Unloading Lance Dataset](/guides/unload-data/unload-lance).
