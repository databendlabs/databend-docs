...
| Function | Description | Example |
|----------|-------------|---------|
| [ST_UNION_AGG](aggregate-st-union-agg.md) | Unions GEOMETRY values across a group | `ST_UNION_AGG(geom)` → `MULTIPOINT(...)` |
| [ST_INTERSECTION_AGG](aggregate-st-intersection-agg.md) | Intersects GEOMETRY values across a group | `ST_INTERSECTION_AGG(geom)` → `POLYGON(...)` |
| [ST_ENVELOPE_AGG](aggregate-st-envelope-agg.md) | Returns the bounding rectangle of GEOMETRY values in a group | `ST_ENVELOPE_AGG(geom)` → `POLYGON(...)` |
| [ST_COLLECT](aggregate-st-collect.md) | Collects GEOMETRY values into one GEOMETRY result | `ST_COLLECT(geom)` → `GEOMETRYCOLLECTION(...)` |
